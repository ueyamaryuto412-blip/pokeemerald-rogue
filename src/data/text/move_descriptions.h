
#if B_BINDING_TURNS >= GEN_5
#define BINDING_TURNS "4 or 5"
#else
#define BINDING_TURNS "2 to 5"
#endif

static const u8 sNullDescription[] = _(
    "");

static const u8 sPoundDescription[] = _("{JPN}ながい しっぽや てなどを つかって\nあいてを はたいて こうげきする。");

static const u8 sKarateChopDescription[] = _("{JPN}するどい チョップで こうげき。\nきゅうしょに あたりやすい。");

static const u8 sDoubleSlapDescription[] = _("{JPN}おうふく ビンタで あいてを たたいて\nこうげきする。");

static const u8 sCometPunchDescription[] = _("{JPN}どとうの パンチで あいてを\nなぐりつけて こうげきする。");

static const u8 sMegaPunchDescription[] = _("{JPN}ちからを こめた パンチで あいてを\nこうげきする。");

static const u8 sPayDayDescription[] = _("{JPN}あいての からだに こばんを\nなげつけて こうげきする。");

static const u8 sFirePunchDescription[] = _("{JPN}ほのおの パンチ。 やけどに\nすることが ある。");

static const u8 sIcePunchDescription[] = _("{JPN}れいきの パンチ。 こおりに\nすることが ある。");

static const u8 sThunderPunchDescription[] = _("{JPN}でんげきを こめた パンチで あいてを\nこうげきする。");

static const u8 sScratchDescription[] = _("{JPN}かたく とがった するどい ツメで\nあいてを ひっかいて こうげきする。");

static const u8 sViseGripDescription[] = _("{JPN}あいてを りょうがわから はさんで\nダメージを あたえる。");

static const u8 sGuillotineDescription[] = _("{JPN}おおきな ハサミで あいてを\nきりさいて こうげきする。");

static const u8 sRazorWindDescription[] = _("{JPN}2ターンめに こうげき。 きゅうしょに\nあたりやすい。");

static const u8 sSwordsDanceDescription[] = _("{JPN}はげしく おどり じぶんの こうげきを\nぐーんと あげる。");

static const u8 sCutDescription[] = _("{JPN}カマや ツメなどで あいてを\nきりつけて こうげきする。");

static const u8 sGustDescription[] = _("{JPN}つばさで おこした はげしい かぜを\nあいてに ぶつけて こうげきする。");

static const u8 sWingAttackDescription[] = _("{JPN}おおきく ひろげた りっぱな つばさを\nあいてに ぶつけて こうげきする。");

static const u8 sWhirlwindDescription[] = _("{JPN}あいてを ふきとばして ひかえの\nポケモンを ひきずりだす。");

static const u8 sFlyDescription[] = _("{JPN}1ターンめで そらへ とび\n2ターンめに あいてを こうげきする。");

static const u8 sBindDescription[] = _(
    "Binds and squeezes the foe\n"
    "for "BINDING_TURNS" turns.");

static const u8 sSlamDescription[] = _("{JPN}ながい しっぽや つるなどを つかい\nあいてを たたきつけて こうげきする。");

static const u8 sVineWhipDescription[] = _("{JPN}しなる つるで あいてを たたきつけて\nこうげきする。");

static const u8 sStompDescription[] = _("{JPN}ふみつけて こうげき。 ひるませる\nことが ある。");

static const u8 sDoubleKickDescription[] = _("{JPN}2ほんの あしで あいてを けとばして\nこうげきする。");

static const u8 sMegaKickDescription[] = _("{JPN}ものすごい ちからを こめた キックで\nあいてを けとばして こうげきする。");

static const u8 sJumpKickDescription[] = _("{JPN}たかい ジャンプからの キックで\nあいてを こうげきする。");

static const u8 sRollingKickDescription[] = _("{JPN}かいてん して けとばす。 ひるませる\nことが ある。");

static const u8 sSandAttackDescription[] = _("{JPN}あいての かおに すなを かけて\nめいちゅうりつを さげる。");

static const u8 sHeadbuttDescription[] = _("{JPN}あたまから つっこむ。 ひるませる\nことが ある。");

static const u8 sHornAttackDescription[] = _("{JPN}するどく とがった つので あいてを\nこうげきする。");

static const u8 sFuryAttackDescription[] = _("{JPN}つのや くちばしで あいてを つついて\nこうげきする。");

static const u8 sHornDrillDescription[] = _("{JPN}かいてんする つのを あいてに\nつきさして こうげきする。");

static const u8 sTackleDescription[] = _("{JPN}あいてに むかって からだ ぜんたいで\nぶつかって いき こうげきする。");

static const u8 sBodySlamDescription[] = _("{JPN}からだ ぜんたいで あいてに\nのしかかり こうげきする。");

static const u8 sWrapDescription[] = _(
    "Wraps and squeezes the foe\n"
    BINDING_TURNS" times with vines, etc.");

static const u8 sTakeDownDescription[] = _("{JPN}すごい いきおいで あいてに\nぶつかって こうげきする。");

static const u8 sThrashDescription[] = _("{JPN}2から 3ターン あばれて\nそのあと こんらんする。");

static const u8 sDoubleEdgeDescription[] = _("{JPN}いのちを かけて あいてに\nとっしんして こうげきする。");

static const u8 sTailWhipDescription[] = _("{JPN}しっぽを ふって ゆだんを さそい\nぼうぎょを さげる。");

static const u8 sPoisonStingDescription[] = _("{JPN}どくの ある ハリを あいてに\nつきさして こうげきする。");

static const u8 sTwineedleDescription[] = _("{JPN}2ほんの ハリを あいてに つきさし\n2かい れんぞくで ダメージ。");

static const u8 sPinMissileDescription[] = _("{JPN}するどい ハリを あいてに\nはっしゃして こうげきする。");

static const u8 sLeerDescription[] = _("{JPN}するどい めつきで おびえさせて\nあいての ぼうぎょを さげる。");

static const u8 sBiteDescription[] = _("{JPN}するどい はで かみつく。 ひるませる\nことが ある。");

static const u8 sGrowlDescription[] = _("{JPN}かわいい なきごえで あいての\nこうげきを さげる。");

static const u8 sRoarDescription[] = _("{JPN}あいてを にがして ひかえの\nポケモンを ひきずりだす。");

static const u8 sSingDescription[] = _("{JPN}きれいな うたごえで あいてを\nねむらせる。");

static const u8 sSupersonicDescription[] = _("{JPN}とくしゅな おんぱを からだから\nはっして あいてを こんらん させる。");

static const u8 sSonicBoomDescription[] = _("{JPN}しょうげきはを ぶつけて 20の\nダメージを あたえる。");

static const u8 sDisableDescription[] = _("{JPN}あいてが ちょくぜんに だした わざを\n4ターン ふうじる。");

static const u8 sAcidDescription[] = _(
    "Sprays a hide-melting acid.\n"
#if B_UPDATED_MOVE_DATA >= GEN_4
    "May lower Sp. Def.");
#else
    "May lower Defense.");
#endif

static const u8 sEmberDescription[] = _("{JPN}ちいさな ほのおを あいてに\nはっしゃして こうげきする。");

static const u8 sFlamethrowerDescription[] = _("{JPN}はげしい ほのおを あいてに\nはっしゃして こうげきする。");

static const u8 sMistDescription[] = _("{JPN}5ターンの あいだ のうりょくを\nさげられなく なる。");

static const u8 sWaterGunDescription[] = _("{JPN}みずを いきおいよく あいてに\nはっしゃして こうげきする。");

static const u8 sHydroPumpDescription[] = _("{JPN}たいりょうの みずを はげしく\nぶつけて こうげき。");

static const u8 sSurfDescription[] = _("{JPN}おおきな なみで じぶんの まわりに\nいるものを こうげきする。");

static const u8 sIceBeamDescription[] = _("{JPN}こごえる ビームを あいてに\nはっしゃして こうげきする。");

static const u8 sBlizzardDescription[] = _("{JPN}こおりの あらしで こうげき。\nしもやけに することが ある。");

static const u8 sPsybeamDescription[] = _("{JPN}ふしぎな こうせん。 こんらん\nさせることが ある。");

static const u8 sBubbleBeamDescription[] = _("{JPN}あわを ぶつける。 すばやさを さげる\nことが ある。");

static const u8 sAuroraBeamDescription[] = _("{JPN}にじいろの ビーム。 こうげきを\nさげる ことが ある。");

static const u8 sHyperBeamDescription[] = _("{JPN}いりょくは たかいが つぎの\nターンは うごけなく なる。");

static const u8 sPeckDescription[] = _("{JPN}するどく とがった くちばしや つので\nあいてを つついて こうげきする。");

static const u8 sDrillPeckDescription[] = _("{JPN}かいてん しながら くちばしを\nつきさして こうげき。");

static const u8 sSubmissionDescription[] = _("{JPN}じめんに じぶんごと あいてを\nなげつけて こうげきする。");

static const u8 sLowKickDescription[] = _("{JPN}あしを つよく けり あいてを\nころばせて こうげきする。");

static const u8 sCounterDescription[] = _("{JPN}うけた ぶつり こうげきの ダメージを\nばいに して かえす。");

static const u8 sSeismicTossDescription[] = _("{JPN}じぶんの レベルと おなじ\nダメージを あたえる。");

static const u8 sStrengthDescription[] = _("{JPN}こんしんの ちからで あいてを\nなぐりつけて こうげきする。");

static const u8 sAbsorbDescription[] = _("{JPN}あいてに あたえた ダメージの\nはんぶんの HPを かいふくできる。");

static const u8 sMegaDrainDescription[] = _("{JPN}あいてに あたえた ダメージの\nはんぶんの HPを かいふくできる。");

static const u8 sLeechSeedDescription[] = _("{JPN}まいターン あいての HPを すいとり\nじぶんが かいふく。");

static const u8 sGrowthDescription[] = _("{JPN}おおきく そだって こうげきと\nとくこうを あげる。");

static const u8 sRazorLeafDescription[] = _("{JPN}はっぱで きりつける。 きゅうしょに\nあたりやすい。");

static const u8 sSolarBeamDescription[] = _("{JPN}1ターンめに ひかりを あつめ\n2ターンめに こうげきする。");

static const u8 sPoisonPowderDescription[] = _("{JPN}どくの こなを まいて あいてを\nどくに する。");

static const u8 sStunSporeDescription[] = _("{JPN}しびれる こなを たくさん ふりまいて\nあいてを まひ じょうたいに する。");

static const u8 sSleepPowderDescription[] = _("{JPN}ねむけの こなを まいて あいてを\nねむらせる。");

static const u8 sPetalDanceDescription[] = _("{JPN}2から 3ターン あばれて\nそのあと こんらんする。");

static const u8 sStringShotDescription[] = _("{JPN}いとを まきつけ すばやさを がくっと\nさげる。");

static const u8 sDragonRageDescription[] = _("{JPN}いかりの しょうげきはを あいてに\nぶつけて こうげきする。");

static const u8 sFireSpinDescription[] = _(
    "Traps the foe in a ring of\n"
    "fire for "BINDING_TURNS" turns.");

static const u8 sThunderShockDescription[] = _("{JPN}でんきの しげきを あいてに あびせて\nこうげきする。");

static const u8 sThunderboltDescription[] = _("{JPN}つよい でんげきを あいてに あびせて\nこうげきする。");

static const u8 sThunderWaveDescription[] = _("{JPN}よわい でんげきを あびせる ことで\nあいてを まひ じょうたいに する。");

static const u8 sThunderDescription[] = _("{JPN}はげしい かみなりを あいてに\nおとして こうげきする。");

static const u8 sRockThrowDescription[] = _("{JPN}ちいさな いわを もちあげて あいてに\nなげつけて こうげきする。");

static const u8 sEarthquakeDescription[] = _("{JPN}じしんの しょうげきで じぶんの\nまわりに いるものを こうげきする。");

static const u8 sFissureDescription[] = _("{JPN}じわれの さけめに あいてを おとして\nこうげきする。");

static const u8 sDigDescription[] = _("{JPN}1ターンめに もぐり 2ターンめで\nあいてを こうげきする。");

static const u8 sToxicDescription[] = _("{JPN}もうどくに する。 ターンが\nすすむほど ダメージが ふえる。");

static const u8 sConfusionDescription[] = _("{JPN}よわい ねんりきを あいてに おくって\nこうげきする。");

static const u8 sPsychicDescription[] = _("{JPN}つよい ねんりきを あいてに おくって\nこうげきする。");

static const u8 sHypnosisDescription[] = _("{JPN}ねむけを さそう あんじを かけて\nあいてを ねむり じょうたいに する。");

static const u8 sMeditateDescription[] = _("{JPN}ちからを ひきだして じぶんの\nこうげきを あげる。");

static const u8 sAgilityDescription[] = _("{JPN}からだを かるくして すばやさを\nぐーんと あげる。");

static const u8 sQuickAttackDescription[] = _("{JPN}ものすごい はやさで つっこむ。\nかならず せんせい。");

static const u8 sRageDescription[] = _("{JPN}わざの あと こうげきを うけると\nこうげきが あがる。");

static const u8 sTeleportDescription[] = _("{JPN}ひかえと いれかわる。 やせいの\nポケモンは にげる。");

static const u8 sNightShadeDescription[] = _("{JPN}じぶんの レベルと おなじ\nダメージを あたえる。");

static const u8 sMimicDescription[] = _("{JPN}あいてが さいごに つかった わざを\nじぶんの わざに する。");

static const u8 sScreechDescription[] = _("{JPN}いやなおとで あいての ぼうぎょを\nがくっと さげる。");

static const u8 sDoubleTeamDescription[] = _("{JPN}ぶんしんを つくり じぶんの\nかいひりつを あげる。");

static const u8 sRecoverDescription[] = _("{JPN}さいだい HPの はんぶんまで\nたいりょくを かいふくする。");

static const u8 sHardenDescription[] = _("{JPN}からだを かたくして じぶんの\nぼうぎょを あげる。");

static const u8 sMinimizeDescription[] = _("{JPN}ちいさく なって かいひりつを\nぐーんと あげる。");

static const u8 sSmokescreenDescription[] = _("{JPN}けむりや スミなどを ふきかけて\nあいての めいちゅうりつを さげる。");

static const u8 sConfuseRayDescription[] = _("{JPN}あやしい ひかりで あいてを こんらん\nさせる。");

static const u8 sWithdrawDescription[] = _("{JPN}からに もぐりこんで みをまもり\nじぶんの ぼうぎょを あげる。");

static const u8 sDefenseCurlDescription[] = _("{JPN}からだを まるめて ちぢこまり\nじぶんの ぼうぎょを あげる。");

static const u8 sBarrierDescription[] = _("{JPN}かべを つくり じぶんの ぼうぎょを\nぐーんと あげる。");

static const u8 sLightScreenDescription[] = _("{JPN}5ターン とくしゅ こうげきの\nダメージを よわめる。");

static const u8 sHazeDescription[] = _("{JPN}せんとうちゅう ぜんいんの\nのうりょくを もとに もどす。");

static const u8 sReflectDescription[] = _("{JPN}5ターン ぶつり こうげきの\nダメージを よわめる。");

static const u8 sFocusEnergyDescription[] = _("{JPN}きあいを こめて きゅうしょに\nあたりやすく なる。");

static const u8 sBideDescription[] = _("{JPN}2ターン たえて うけた ダメージを\nばいに して かえす。");

static const u8 sMetronomeDescription[] = _("{JPN}すべての わざの なかから どれか\n1つを くりだす。");

static const u8 sMirrorMoveDescription[] = _("{JPN}あいての つかった わざを まねして\nじぶんも おなじ わざを つかう。");

static const u8 sSelfDestructDescription[] = _("{JPN}おおきな ダメージを あたえるが\nじぶんは ひんしに なる。");

static const u8 sEggBombDescription[] = _("{JPN}おおきな タマゴを ちから いっぱい\nあいてに なげつけて こうげきする。");

static const u8 sLickDescription[] = _("{JPN}ながい したで あいてを なめまわして\nこうげきする。");

static const u8 sSmogDescription[] = _("{JPN}よごれた ガスを あいてに ふきつけて\nこうげきする。");

static const u8 sSludgeDescription[] = _("{JPN}きたない ヘドロを あいてに\nなげつけて こうげきする。");

static const u8 sBoneClubDescription[] = _("{JPN}ホネで なぐる。 ひるませる ことが\nある。");

static const u8 sFireBlastDescription[] = _("{JPN}だいのじの ほのお。 やけどに\nすることが ある。");

static const u8 sWaterfallDescription[] = _("{JPN}すごい いきおいで つっこむ。\nひるませる ことが ある。");

static const u8 sClampDescription[] = _(
    "Traps and squeezes the\n"
    "foe for "BINDING_TURNS" turns.");

static const u8 sSwiftDescription[] = _("{JPN}ほしがたの ひかりを はっしゃして\nあいてを こうげきする。");

static const u8 sSkullBashDescription[] = _("{JPN}1ターンめに ぼうぎょを あげ\n2ターンめに こうげき。");

static const u8 sSpikeCannonDescription[] = _("{JPN}するどい ハリを あいてに\nはっしゃして こうげきする。");

static const u8 sConstrictDescription[] = _("{JPN}しょくしゅや ツタ などを\nからみつけて こうげきする。");

static const u8 sAmnesiaDescription[] = _("{JPN}あたまを からにして とくぼうを\nぐーんと あげる。");

static const u8 sKinesisDescription[] = _("{JPN}スプーンを まげて ちゅういを ひき\nあいての めいちゅうりつを さげる。");

static const u8 sSoftBoiledDescription[] = _("{JPN}さいだい HPの はんぶんまで\nたいりょくを かいふくする。");

static const u8 sHighJumpKickDescription[] = _("{JPN}ひざげりで こうげき。 はずすと\nじぶんが きずつく。");

static const u8 sGlareDescription[] = _("{JPN}おなかの もようで おびえさせて\nあいてを まひの じょうたいに する。");

static const u8 sDreamEaterDescription[] = _("{JPN}ねている あいてを こうげきし\nダメージの はんぶん かいふく。");

static const u8 sPoisonGasDescription[] = _("{JPN}どくガスを ふきかけて あいてを\nどくに する。");

static const u8 sBarrageDescription[] = _("{JPN}まるい ものを なげつけ 2ー5かい\nつづけて あたる。");

static const u8 sLeechLifeDescription[] = _("{JPN}あたえた ダメージの はんぶんを\nすいとる こうげき。");

static const u8 sLovelyKissDescription[] = _("{JPN}おそろしい かおで キスを せまる。\nあいてを ねむり じょうたいに する。");

static const u8 sSkyAttackDescription[] = _("{JPN}2ターンめに こうげき。 ひるませる\nことも ある。");

static const u8 sTransformDescription[] = _("{JPN}あいてに へんしんして おなじ わざが\nつかえる。");

static const u8 sBubbleDescription[] = _("{JPN}むすうの あわを あいてに ふきかけて\nこうげきする。");

static const u8 sDizzyPunchDescription[] = _("{JPN}リズムの パンチ。 こんらん\nさせることが ある。");

static const u8 sSporeDescription[] = _("{JPN}ほうしを まいて あいてを ねむり\nじょうたいに する。");

static const u8 sFlashDescription[] = _("{JPN}まぶしい ひかりで あいての\nめいちゅうりつを さげる。");

static const u8 sPsywaveDescription[] = _("{JPN}ふしぎな ねんぱで こうげき。\nダメージが かわる。");

static const u8 sSplashDescription[] = _("{JPN}ただ はねるだけ…\nなんの こうかも ない。");

static const u8 sAcidArmorDescription[] = _("{JPN}えきじょうに なって ぼうぎょを\nぐーんと あげる。");

static const u8 sCrabhammerDescription[] = _("{JPN}ハサミで たたく。 きゅうしょに\nあたりやすい。");

static const u8 sExplosionDescription[] = _("{JPN}おおきな ダメージを あたえるが\nじぶんは ひんしに なる。");

static const u8 sFurySwipesDescription[] = _("{JPN}ツメや カマなどで あいてを\nひっかいて こうげきする。");

static const u8 sBonemerangDescription[] = _("{JPN}ホネを なげつけ いきと かえりの\n2かい あたる。");

static const u8 sRestDescription[] = _("{JPN}じぶんの HPと じょうたいいじょうを\nすべて かいふく する。");

static const u8 sRockSlideDescription[] = _("{JPN}おおきな いわを ぶつける。\nひるませる ことが ある。");

static const u8 sHyperFangDescription[] = _("{JPN}まえばで かみつく。 ひるませる\nことが ある。");

static const u8 sSharpenDescription[] = _("{JPN}カクカクに なって じぶんの\nこうげきを あげる。");

static const u8 sConversionDescription[] = _("{JPN}いちばん うえの わざと おなじ\nタイプに なる。");

static const u8 sTriAttackDescription[] = _("{JPN}3つの こうせん。 まひ やけど\nこおりの どれかに する。");

static const u8 sSuperFangDescription[] = _("{JPN}はげしく かみつき あいての HPを\nはんぶんに する。");

static const u8 sSlashDescription[] = _("{JPN}きりさいて こうげき。 きゅうしょに\nあたりやすい。");

static const u8 sSubstituteDescription[] = _("{JPN}HPを けずって ぶんしんを だし\nみがわりに する。");

static const u8 sStruggleDescription[] = _("{JPN}じぶんの PPが なくなると あがいて\nあいてを こうげきする。");

static const u8 sSketchDescription[] = _("{JPN}あいての わざを じぶんの ものに\nする。 1かいだけ。");

static const u8 sTripleKickDescription[] = _("{JPN}3かい れんぞくで キックを\nくりだして こうげきする。");

static const u8 sThiefDescription[] = _("{JPN}こうげきと どうじに あいての\nどうぐを ぬすむ。");

static const u8 sSpiderWebDescription[] = _("{JPN}いとを からませ あいてを\nにげられなく する。");

static const u8 sMindReaderDescription[] = _("{JPN}つぎの こうげきが かならず\nあたるように する。");

static const u8 sNightmareDescription[] = _("{JPN}ねむっている あいての HPを\nまいターン へらす。");

static const u8 sFlameWheelDescription[] = _("{JPN}ほのおを まとい あいてに\nとっしんして こうげきする。");

static const u8 sSnoreDescription[] = _("{JPN}ねながら こうげき。 ひるませる\nことが ある。");

static const u8 sCurseDescription[] = _("{JPN}つかう ポケモンが ゴーストタイプと\nそれいがい とでは こうかが かわる。");

static const u8 sFlailDescription[] = _("{JPN}じぶんの HPが すくないほど\nダメージが おおきく なる。");

static const u8 sConversion2Description[] = _("{JPN}あいての さいごの わざに つよい\nタイプに かわる。");

static const u8 sAeroblastDescription[] = _("{JPN}くうきの うずで こうげき。\nきゅうしょに あたりやすい。");

static const u8 sCottonSporeDescription[] = _("{JPN}ほうしを まとわせ すばやさを\nがくっと さげる。");

static const u8 sReversalDescription[] = _("{JPN}じぶんの HPが すくないほど\nダメージが おおきく なる。");

static const u8 sSpiteDescription[] = _("{JPN}あいてが さいごに つかった わざの\nPPを 4 へらす。");

static const u8 sPowderSnowDescription[] = _("{JPN}つめたい こなゆきを あいてに\nふきつけて こうげきする。");

static const u8 sProtectDescription[] = _("{JPN}こうげきを うけない。 つづけると\nしっぱい しやすい。");

static const u8 sMachPunchDescription[] = _("{JPN}ものすごい はやさの パンチ。\nかならず せんせい。");

static const u8 sScaryFaceDescription[] = _("{JPN}にらんで おびえさせ すばやさを\nがくっと さげる。");

static const u8 sFeintAttackDescription[] = _("{JPN}ゆだんした すきを ついて\nなぐりつける。");

static const u8 sSweetKissDescription[] = _("{JPN}てんしのように かわいく キスして\nあいてを こんらん させる。");

static const u8 sBellyDrumDescription[] = _("{JPN}HPを はんぶん へらし こうげきを\nさいだいに あげる。");

static const u8 sSludgeBombDescription[] = _("{JPN}きたない ヘドロを あいてに\nなげつけて こうげきする。");

static const u8 sMudSlapDescription[] = _("{JPN}ドロを なげつけ めいちゅうりつを\nさげる。");

static const u8 sOctazookaDescription[] = _("{JPN}あいての かおなどに すみを\nふきかけて こうげきする。");

static const u8 sSpikesDescription[] = _("{JPN}こうたいで でてきた あいての\nポケモンに ダメージを あたえる。");

static const u8 sZapCannonDescription[] = _("{JPN}たいほうの ような でんきを\nはっしゃして こうげきする。");

static const u8 sForesightDescription[] = _("{JPN}ゴーストにも あたるように なり\nかいひりつを むし。");

static const u8 sDestinyBondDescription[] = _("{JPN}わざの あと ひんしに なると\nあいても ひんしに する。");

static const u8 sPerishSongDescription[] = _("{JPN}うたを きいた ポケモンは 3ターン\nたつと ひんしに なる。");

static const u8 sIcyWindDescription[] = _("{JPN}つめたい れいきで こうげき。\nすばやさを さげる。");

static const u8 sDetectDescription[] = _("{JPN}こうげきを うけない。 つづけると\nしっぱい しやすい。");

static const u8 sBoneRushDescription[] = _("{JPN}かたい ホネで あいてを なぐりつけて\nこうげきする。");

static const u8 sLockOnDescription[] = _("{JPN}つぎの こうげきが かならず\nあたるように する。");

static const u8 sOutrageDescription[] = _("{JPN}2から 3ターン あばれて\nそのあと こんらんする。");

static const u8 sSandstormDescription[] = _("{JPN}5ターン すなあらし。 いわ じめん\nはがね いがいに ダメージ。");

static const u8 sGigaDrainDescription[] = _("{JPN}あたえた ダメージの はんぶんを\nすいとる こうげき。");

static const u8 sEndureDescription[] = _("{JPN}HPを かならず 1 のこす。\nつづけると しっぱい しやすい。");

static const u8 sCharmDescription[] = _("{JPN}かわいく みつめて こうげきを\nがくっと さげる。");

static const u8 sRolloutDescription[] = _("{JPN}5ターン ころがる。 あたる たびに\nいりょくが あがる。");

static const u8 sFalseSwipeDescription[] = _("{JPN}あいての HPが かならず 1\nのこるように こうげき。");

static const u8 sSwaggerDescription[] = _("{JPN}あいてを こんらん させるが\nこうげきを ぐーんと あげる。");

static const u8 sMilkDrinkDescription[] = _("{JPN}さいだい HPの はんぶんまで\nたいりょくを かいふくする。");

static const u8 sSparkDescription[] = _("{JPN}でんきを まとい あいてに\nとっしんして こうげきする。");

static const u8 sFuryCutterDescription[] = _("{JPN}カマや ツメ などで あいてを\nきりつけて こうげきする。");

static const u8 sSteelWingDescription[] = _("{JPN}かたい つばさを あいてに\nたたきつけて こうげきする。");

static const u8 sMeanLookDescription[] = _("{JPN}こわい かおで にらみつけて\nあいてを にげられなく する。");

static const u8 sAttractDescription[] = _("{JPN}あいてを メロメロに して わざを\nだしにくく する。");

static const u8 sSleepTalkDescription[] = _("{JPN}ねている ときだけ つかえる。\nおぼえた わざを 1つ だす。");

static const u8 sHealBellDescription[] = _("{JPN}すずの ねいろで みかたの いじょうを\nかいふく する。");

static const u8 sReturnDescription[] = _("{JPN}トレーナーの ために ぜんりょくで\nあいてを こうげきする。");

static const u8 sPresentDescription[] = _("{JPN}わなを しかけた はこを あいてに\nわたして こうげきする。");

static const u8 sFrustrationDescription[] = _("{JPN}ふまんを はらすため ぜんりょくで\nあいてを こうげきする。");

static const u8 sSafeguardDescription[] = _("{JPN}5ターンの あいだ じょうたい\nいじょうに ならない。");

static const u8 sPainSplitDescription[] = _("{JPN}じぶんと あいての HPを あわせて\nなかよく わける。");

static const u8 sSacredFireDescription[] = _("{JPN}しんぴの ほのおで あいてを\nやきつくして こうげきする。");

static const u8 sMagnitudeDescription[] = _("{JPN}じめんを ゆらして じぶんの まわりに\nいるものを こうげきする。");

static const u8 sDynamicPunchDescription[] = _("{JPN}こんしんの ちからで パンチを\nくりだして こうげきする。");

static const u8 sMegahornDescription[] = _("{JPN}りっぱな つので おもいっきり\nつきさして こうげき。");

static const u8 sDragonBreathDescription[] = _("{JPN}ものすごい いきを あいてに\nふきつけて こうげきする。");

static const u8 sBatonPassDescription[] = _("{JPN}ひかえと いれかわる。\nのうりょくへんかは ひきつぐ。");

static const u8 sEncoreDescription[] = _("{JPN}あいてに アンコールした わざを\n3かい つづけて ださせる。");

static const u8 sPursuitDescription[] = _("{JPN}あいてが いれかわる ときに だすと\nいりょくが ばいに。");

static const u8 sRapidSpinDescription[] = _("{JPN}かいてん こうげき。 しめつけや\nやどりぎを ふきとばす。");

static const u8 sSweetScentDescription[] = _("{JPN}かおりで あいての かいひりつを\nがくっと さげる。");

static const u8 sIronTailDescription[] = _("{JPN}かたい しっぽで あいてを\nたたきつけて こうげきする。");

static const u8 sMetalClawDescription[] = _("{JPN}こうてつの ツメで あいてを\nきりさいて こうげきする。");

static const u8 sVitalThrowDescription[] = _("{JPN}あとから こうげき。 かならず\nめいちゅう する。");

static const u8 sMorningSunDescription[] = _("{JPN}HPを かいふく。 てんきで\nかいふくりょうが かわる。");

static const u8 sSynthesisDescription[] = _("{JPN}HPを かいふく。 てんきで\nかいふくりょうが かわる。");

static const u8 sMoonlightDescription[] = _("{JPN}HPを かいふく。 てんきで\nかいふくりょうが かわる。");

static const u8 sHiddenPowerDescription[] = _("{JPN}わざを つかった ポケモンに よって\nわざの タイプが かわる。");

static const u8 sCrossChopDescription[] = _("{JPN}りょうての チョップ。 きゅうしょに\nあたりやすい。");

static const u8 sTwisterDescription[] = _("{JPN}たつまきで こうげき。 ひるませる\nことが ある。");

static const u8 sRainDanceDescription[] = _("{JPN}5ターンの あいだ あめを ふらせて\nみずタイプの いりょくを あげる。");

static const u8 sSunnyDayDescription[] = _("{JPN}5ターン ひざしを つよくし ほのおを\nつよくする。");

static const u8 sCrunchDescription[] = _(
    "Crunches with sharp fangs.\n"
#if B_UPDATED_MOVE_DATA >= GEN_4
    "May lower Defense.");
#else
    "May lower Sp. Def.");
#endif

static const u8 sMirrorCoatDescription[] = _("{JPN}うけた とくしゅ こうげきを ばいに\nして かえす。");

static const u8 sPsychUpDescription[] = _("{JPN}のうりょくへんかを あいてと おなじに\nする。");

static const u8 sExtremeSpeedDescription[] = _("{JPN}めにも とまらぬ ものすごい はやさで\nあいてに とっしんして こうげきする。");

static const u8 sAncientPowerDescription[] = _("{JPN}げんしの ちから。 すべての\nのうりょくが あがることが ある。");

static const u8 sShadowBallDescription[] = _("{JPN}くろいかげを なげつける。 とくぼうを\nさげることが ある。");

static const u8 sFutureSightDescription[] = _("{JPN}2ターン あとに ねんりきの\nかたまりで こうげき。");

static const u8 sRockSmashDescription[] = _("{JPN}パンチで こうげき。 あいての\nぼうぎょを さげる ことが ある。");

static const u8 sWhirlpoolDescription[] = _(
    "Traps and hurts the foe in\n"
    "a whirlpool for "BINDING_TURNS" turns.");

static const u8 sBeatUpDescription[] = _("{JPN}なかまの ポケモンが おおいほど\nわざの こうげきかいすうが ふえる。");

static const u8 sFakeOutDescription[] = _("{JPN}せんとうに でたら すぐに ださないと\nせいこう しない。");

static const u8 sUproarDescription[] = _(
#if B_UPROAR_TURNS >= GEN_5
    "Causes an uproar for 2 to 5\n"
#else
    "Causes an uproar for 3\n"
#endif
    "turns and prevents sleep.");

static const u8 sStockpileDescription[] = _("{JPN}ちからを たくわえて じぶんの\nぼうぎょと とくぼうを あげる。");

static const u8 sSpitUpDescription[] = _("{JPN}たくわえた ちからを あいてに\nぶつけて こうげきする。");

static const u8 sSwallowDescription[] = _("{JPN}たくわえた ちからを のみこんで\nじぶんの HPを かいふく する。");

static const u8 sHeatWaveDescription[] = _("{JPN}あつい いきで こうげき。 やけどに\nすることが ある。");

static const u8 sHailDescription[] = _("{JPN}あられを ふらせて まいターン\nダメージを あたえる。");

static const u8 sTormentDescription[] = _("{JPN}あいてが おなじ わざを 2かい\nつづけて だせなく なる。");

static const u8 sFlatterDescription[] = _("{JPN}あいてを こんらん させるが\nとくこうも あげて しまう。");

static const u8 sWillOWispDescription[] = _("{JPN}あやしい ほのおで あいてを やけどの\nじょうたいに する。");

static const u8 sMementoDescription[] = _("{JPN}じぶんは ひんしに なり あいての\nこうげきと とくこうを さげる。");

static const u8 sFacadeDescription[] = _("{JPN}やけど まひ どくの ときに\nこうげきが あがる。");

static const u8 sFocusPunchDescription[] = _("{JPN}パンチを くりだす。 だす まえに\nうけると しっぱい。");

static const u8 sSmellingSaltsDescription[] = _("{JPN}まひの あいてには ばいの いりょく。\nまひは なおる。");

static const u8 sFollowMeDescription[] = _("{JPN}あいての こうげきを すべて じぶんに\nむけさせる。");

static const u8 sNaturePowerDescription[] = _("{JPN}しぜんの ちからで こうげき。\nばしょで わざが かわる。");

static const u8 sChargeDescription[] = _("{JPN}つぎの でんきわざを つよくし\nとくぼうも あがる。");

static const u8 sTauntDescription[] = _("{JPN}3ターン あいては ダメージわざしか\nだせなく なる。");

static const u8 sHelpingHandDescription[] = _("{JPN}なかまの わざの いりょくを おおきく\nする。");

static const u8 sTrickDescription[] = _("{JPN}あいての すきを ついて じぶんと\nあいての もちものを こうかんする。");

static const u8 sRolePlayDescription[] = _("{JPN}あいてと おなじ とくせいに\nへんかする。");

static const u8 sWishDescription[] = _("{JPN}HPを かいふくする ねがい。\nきくまでに じかんが かかる。");

static const u8 sAssistDescription[] = _("{JPN}なかまの わざの どれかを\nランダムに つかって こうげき。");

static const u8 sIngrainDescription[] = _("{JPN}だいちに ねを はり まいターン\nじぶんの HPを かいふく する。");

static const u8 sSuperpowerDescription[] = _("{JPN}すごい ちからを はっきして あいてを\nこうげきする。");

static const u8 sMagicCoatDescription[] = _("{JPN}じょうたい いじょうの わざなどを\nあいてに はねかえす。");

static const u8 sRecycleDescription[] = _("{JPN}なくなった もちものを さいせい\nさせて つかえるように する。");

static const u8 sRevengeDescription[] = _("{JPN}わざを うけて いると あたえる\nダメージが ばいに なる。");

static const u8 sBrickBreakDescription[] = _("{JPN}しゅとうを いきおいよく ふりおろして\nあいてを こうげきする。");

static const u8 sYawnDescription[] = _("{JPN}あくびを して つぎの ターンに\nあいてを ねむらせる。");

static const u8 sKnockOffDescription[] = _("{JPN}もちものを はたきおとし\nせんとうちゅう つかえなくする。");

static const u8 sEndeavorDescription[] = _("{JPN}あいての HPが じぶんと おなじに\nなるよう ダメージ。");

static const u8 sEruptionDescription[] = _("{JPN}じぶんの HPが すくない ほど\nわざの いりょくは さがる。");

static const u8 sSkillSwapDescription[] = _("{JPN}じぶんの とくせいと あいての\nとくせいを いれかえる。");

static const u8 sImprisonDescription[] = _("{JPN}おなじ わざを おぼえた あいてだけ\nそれを つかえなく する。");

static const u8 sRefreshDescription[] = _("{JPN}やすんで どく まひ やけどの\nじょうたいを なおす。");

static const u8 sGrudgeDescription[] = _("{JPN}わざで ひんしに されると その\nわざの PPを 0に する。");

static const u8 sSnatchDescription[] = _("{JPN}あいてが つぎに つかう わざの\nこうかを よこどりする。");

static const u8 sSecretPowerDescription[] = _("{JPN}つかう ばしょで ついかこうか が\nへんかする こうげき。");

static const u8 sDiveDescription[] = _("{JPN}1ターンめで もぐり 2ターンめに\nうきあがって こうげきする。");

static const u8 sArmThrustDescription[] = _("{JPN}ひらいた りょうてで あいてを\nつっぱって こうげきする。");

static const u8 sCamouflageDescription[] = _("{JPN}いる ばしょに あわせて じぶんの\nタイプを かえる。");

static const u8 sTailGlowDescription[] = _("{JPN}ひかりを はなって とくこうを\nぐーんと あげる。");

static const u8 sLusterPurgeDescription[] = _("{JPN}まばゆい ひかりで こうげき。\nとくぼうを さげることが ある。");

static const u8 sMistBallDescription[] = _("{JPN}うもうで つつむ。 とくこうを\nさげることが ある。");

static const u8 sFeatherDanceDescription[] = _("{JPN}うもうを ふりまいて あいての\nからだに からませる。");

static const u8 sTeeterDanceDescription[] = _("{JPN}まわりの ものを こんらん\nじょうたいに させる。");

static const u8 sBlazeKickDescription[] = _("{JPN}やけどに することが あり\nきゅうしょにも あたりやすい。");

static const u8 sMudSportDescription[] = _("{JPN}5ターンの あいだ でんきタイプの\nわざを よわめる。");

static const u8 sIceBallDescription[] = _("{JPN}5ターン こうげき。 あたる たびに\nいりょくが あがる。");

static const u8 sNeedleArmDescription[] = _("{JPN}トゲの うでで なぐる。 ひるませる\nことが ある。");

static const u8 sSlackOffDescription[] = _("{JPN}やすんで HPを さいだいの はんぶん\nかいふく する。");

static const u8 sHyperVoiceDescription[] = _("{JPN}うるさく ひびく おおきな しんどうを\nあいてに あたえて こうげきする。");

static const u8 sPoisonFangDescription[] = _("{JPN}どくの キバで かみつく。 もうどくに\nすることが ある。");

static const u8 sCrushClawDescription[] = _("{JPN}ツメで きりさく。 ぼうぎょを\nさげることが ある。");

static const u8 sBlastBurnDescription[] = _("{JPN}いりょくは たかいが つぎの\nターンは うごけなく なる。");

static const u8 sHydroCannonDescription[] = _("{JPN}いりょくは たかいが つぎの\nターンは うごけなく なる。");

static const u8 sMeteorMashDescription[] = _("{JPN}すいせいの ごとく パンチを\nくりだして あいてを こうげきする。");

static const u8 sAstonishDescription[] = _("{JPN}ふいに おどろかせる。 ひるませる\nことが ある。");

static const u8 sWeatherBallDescription[] = _("{JPN}つかった ときの てんきに よって\nわざの タイプと いりょくが かわる。");

static const u8 sAromatherapyDescription[] = _("{JPN}やすらぐ かおりで みかたの\nいじょうを かいふく する。");

static const u8 sFakeTearsDescription[] = _("{JPN}ないた ふりを して とくぼうを\nがくっと さげる。");

static const u8 sAirCutterDescription[] = _("{JPN}するどい かぜで きりつける。\nきゅうしょに あたりやすい。");

static const u8 sOverheatDescription[] = _("{JPN}フルパワーで こうげき。 じぶんの\nとくこうが がくっと さがる。");

static const u8 sOdorSleuthDescription[] = _("{JPN}ゴーストにも あたるように なり\nかいひりつを むし。");

static const u8 sRockTombDescription[] = _("{JPN}がんせきを なげて あいての\nすばやさを さげる。");

static const u8 sSilverWindDescription[] = _("{JPN}じぶんの すべての のうりょくが\nあがることが ある。");

static const u8 sMetalSoundDescription[] = _("{JPN}いやな おとで あいての とくぼうを\nがくっと さげる。");

static const u8 sGrassWhistleDescription[] = _("{JPN}ふえの ねいろで あいてを ねむりの\nじょうたいに する。");

static const u8 sTickleDescription[] = _("{JPN}くすぐって あいての こうげきと\nぼうぎょを さげる。");

static const u8 sCosmicPowerDescription[] = _("{JPN}しんぴの ちからで ぼうぎょと\nとくぼうを あげる。");

static const u8 sWaterSpoutDescription[] = _("{JPN}しおで こうげき。 HPが\nすくないほど いりょくが さがる。");

static const u8 sSignalBeamDescription[] = _("{JPN}ふしぎな ひかり。 こんらん\nさせることが ある。");

static const u8 sShadowPunchDescription[] = _("{JPN}かげからの パンチ。 かならず\nめいちゅう する。");

static const u8 sExtrasensoryDescription[] = _("{JPN}みえない ちからで こうげき。\nひるませる ことが ある。");

static const u8 sSkyUppercutDescription[] = _("{JPN}たかい アッパーで あいてを\nつきあげて こうげきする。");

static const u8 sSandTombDescription[] = _(
    "Traps and hurts the foe in\n"
    "quicksand for "BINDING_TURNS" turns.");

static const u8 sSheerColdDescription[] = _("{JPN}いちげきで ひんしに する。\nこおりタイプ いがいは あたりにくい。");

static const u8 sMuddyWaterDescription[] = _("{JPN}にごった みずを あいてに\nはっしゃして こうげきする。");

static const u8 sBulletSeedDescription[] = _("{JPN}タネを いきおいよく あいてに\nはっしゃして こうげきする。");

static const u8 sAerialAceDescription[] = _("{JPN}すばやい うごきで あいてを\nほんろうして きりつける。");

static const u8 sIcicleSpearDescription[] = _("{JPN}するどい つららを あいてに\nはっしゃして こうげきする。");

static const u8 sIronDefenseDescription[] = _("{JPN}ひふを かたくして ぼうぎょを\nぐーんと あげる。");

static const u8 sBlockDescription[] = _("{JPN}にげみちを ふさいで あいてを\nにげられなく する。");

static const u8 sHowlDescription[] = _("{JPN}ほえて じぶんと みかたの こうげきを\nあげる。");

static const u8 sDragonClawDescription[] = _("{JPN}するどく とがった きょだいな ツメで\nあいてを きりさいて こうげきする。");

static const u8 sFrenzyPlantDescription[] = _("{JPN}いりょくは たかいが つぎの\nターンは うごけなく なる。");

static const u8 sBulkUpDescription[] = _("{JPN}からだを おおきくして こうげきと\nぼうぎょを あげる。");

static const u8 sBounceDescription[] = _("{JPN}そらたかく とびはねて 2ターンめに\nあいてを こうげきする。");

static const u8 sMudShotDescription[] = _("{JPN}どろの かたまりを あいてに\nなげつけて こうげきする。");

static const u8 sPoisonTailDescription[] = _("{JPN}しっぽで たたく。 どくに することが\nあり きゅうしょにも あたる。");

static const u8 sCovetDescription[] = _("{JPN}あまえながら ちかづき もちものを\nうばう。");

static const u8 sVoltTackleDescription[] = _("{JPN}じぶんも かなり ダメージを うける。\nまひ じょうたいに することが ある。");

static const u8 sMagicalLeafDescription[] = _("{JPN}あいてを ついせきする ふしぎな\nはっぱを まきちらす。");

static const u8 sWaterSportDescription[] = _("{JPN}5ターンの あいだ ほのおタイプの\nわざを よわめる。");

static const u8 sCalmMindDescription[] = _("{JPN}こころを しずめて とくこうと\nとくぼうを あげる。");

static const u8 sLeafBladeDescription[] = _("{JPN}はっぱを つるぎのように あやつり\nあいてを きりつけて こうげきする。");

static const u8 sDragonDanceDescription[] = _("{JPN}まいを おどり こうげきと すばやさを\nあげる。");

static const u8 sRockBlastDescription[] = _("{JPN}かたい がんせきを あいてに\nはっしゃして こうげきする。");

static const u8 sShockWaveDescription[] = _("{JPN}すばやい でんげき。 かならず\nめいちゅう する。");

static const u8 sWaterPulseDescription[] = _("{JPN}みずの しんどうを あいてに あたえて\nこうげきする。");

static const u8 sDoomDesireDescription[] = _("{JPN}2ターン あとに ひかりの たばで\nあいてを こうげき。");

static const u8 sPsychoBoostDescription[] = _("{JPN}フルパワーで こうげき。 じぶんの\nとくこうが がくっと さがる。");

static const u8 sRoostDescription[] = _("{JPN}さいだい HPの はんぶん\nたいりょくを かいふくする。");

static const u8 sGravityDescription[] = _("{JPN}じゅうりょくを つよめて\nうかぶ ちからを うちけす。");

static const u8 sMiracleEyeDescription[] = _("{JPN}かいひりつと あくタイプの\nむこうかを うちけす。");

static const u8 sWakeUpSlapDescription[] = _("{JPN}ねむっている あいてに つよいが\nめを さまさせて しまう。");

static const u8 sHammerArmDescription[] = _("{JPN}こぶしを ふりまわして こうげき。\nすばやさも さげる。");

static const u8 sGyroBallDescription[] = _("{JPN}こうそくかいてんの こうげき。\nあいてが はやいほど つよい。");

static const u8 sHealingWishDescription[] = _("{JPN}じぶんは ひんしに なるが\nこうたいする みかたを かいふく。");

static const u8 sBrineDescription[] = _("{JPN}HPが はんぶん いかの あいてに\n2ばいの ダメージ。");

static const u8 sNaturalGiftDescription[] = _("{JPN}もっている きのみで\nこうかが かわる。");

static const u8 sFeintDescription[] = _("{JPN}まもるなどを つかっている\nあいてにも あたる こうげき。");

static const u8 sPluckDescription[] = _("{JPN}あいての きのみを たべて\nそのこうかを える。");

static const u8 sTailwindDescription[] = _("{JPN}はげしい かぜを おこして\nすばやさを あげる。");

static const u8 sAcupressureDescription[] = _("{JPN}のうりょくの ひとつを\nぐーんと あげる。");

static const u8 sMetalBurstDescription[] = _("{JPN}うけた こうげきを\nより つよく かえす。");

static const u8 sUTurnDescription[] = _("{JPN}ダメージを あたえてから\nこうたいする。");

static const u8 sCloseCombatDescription[] = _("{JPN}つよい こうげきだが\nぼうぎょめんが さがる。");

static const u8 sPaybackDescription[] = _("{JPN}あとに うごくほど\nいりょくが あがる こうげき。");

static const u8 sAssuranceDescription[] = _("{JPN}あいてが ダメージを うけていると\nいりょくが あがる こうげき。");

static const u8 sEmbargoDescription[] = _("{JPN}あいてが どうぐを\nつかえなく する。");

static const u8 sFlingDescription[] = _("{JPN}もっている どうぐで\nこうかが かわる。");

static const u8 sPsychoShiftDescription[] = _("{JPN}じょうたい いじょうを\nあいてに うつす。");

static const u8 sTrumpCardDescription[] = _("{JPN}わざの PPが すくないほど\nダメージが おおきく なる。");

static const u8 sHealBlockDescription[] = _("{JPN}あいてが HPを\nかいふくできなく する。");

static const u8 sWringOutDescription[] = _("{JPN}あいての HPが おおいほど\nダメージが おおきく なる。");

static const u8 sPowerTrickDescription[] = _("{JPN}じぶんの こうげきと ぼうぎょを\nいれかえる。");

static const u8 sGastroAcidDescription[] = _("{JPN}いえきで あいての とくせいを\nきかなく する。");

static const u8 sLuckyChantDescription[] = _("{JPN}あいての きゅうしょへの\nこうげきを ふせぐ。");

static const u8 sMeFirstDescription[] = _("{JPN}あいての こうげきを\nより つよく つかう。");

static const u8 sCopycatDescription[] = _("{JPN}あいてが さいごに つかった\nわざを まねる。");

static const u8 sPowerSwapDescription[] = _("{JPN}こうげきと とくこうの へんかを\nあいてと いれかえる。");

static const u8 sGuardSwapDescription[] = _("{JPN}ぼうぎょと とくぼうの へんかを\nあいてと いれかえる。");

static const u8 sPunishmentDescription[] = _("{JPN}あいての のうりょくが\nあがっているほど つよい。");

static const u8 sLastResortDescription[] = _("{JPN}ほかの わざを すべて\nつかったあとにしか だせない。");

static const u8 sWorrySeedDescription[] = _("{JPN}あいてに たねを うえつけて\nねむれなく する。");

static const u8 sSuckerPunchDescription[] = _("{JPN}あいてが こうげきの じゅんびを\nしていると さきに あたる。");

static const u8 sToxicSpikesDescription[] = _("{JPN}でてきた あいてを どくに する\nびしを ばらまく。");

static const u8 sHeartSwapDescription[] = _("{JPN}のうりょくへんかを\nあいてと いれかえる。");

static const u8 sAquaRingDescription[] = _("{JPN}みずの ベールを つくって\nHPを かいふくする。");

static const u8 sMagnetRiseDescription[] = _("{JPN}じりょくで からだを\nうかせる。");

static const u8 sFlareBlitzDescription[] = _("{JPN}やけどさせることが ある\nとつげき。じぶんも ダメージ。");

static const u8 sForcePalmDescription[] = _("{JPN}しょうげきはの こうげき。\nまひさせることが ある。");

static const u8 sAuraSphereDescription[] = _("{JPN}かならず あたる\nオーラの こうげき。");

static const u8 sRockPolishDescription[] = _("{JPN}からだを みがいて\nすばやさを ぐーんと あげる。");

static const u8 sPoisonJabDescription[] = _("{JPN}つきさす こうげき。\nどくに することが ある。");

static const u8 sDarkPulseDescription[] = _("{JPN}おそろしい オーラの こうげき。\nひるませることが ある。");

static const u8 sNightSlashDescription[] = _("{JPN}できるだけ はやく こうげき。\nきゅうしょに あたりやすい。");

static const u8 sAquaTailDescription[] = _("{JPN}しっぽを なみのように\nふって こうげきする。");

static const u8 sSeedBombDescription[] = _("{JPN}かたい たねを\nつぎつぎ うちだして こうげき。");

static const u8 sAirSlashDescription[] = _("{JPN}かぜの やいばで こうげき。\nひるませることが ある。");

static const u8 sXScissorDescription[] = _("{JPN}かまや つめなどを\nクロスさせて きりさく。");

static const u8 sBugBuzzDescription[] = _("{JPN}おとの こうげき。\nとくぼうを さげることが ある。");

static const u8 sDragonPulseDescription[] = _("{JPN}しょうげきはを おこして\nあいてに ダメージを あたえる。");

static const u8 sDragonRushDescription[] = _("{JPN}おそろしい いきおいで\nとつげき。ひるませることが ある。");

static const u8 sPowerGemDescription[] = _("{JPN}ダイヤのように きらめく\nひかりで こうげきする。");

static const u8 sVacuumWaveDescription[] = _("{JPN}こぶしを まわして おこした\nなみで さきに こうげき。");

static const u8 sFocusBlastDescription[] = _("{JPN}ぜんりょくで こうげき。\nとくぼうを さげることが ある。");

static const u8 sEnergyBallDescription[] = _("{JPN}しぜんの ちからで こうげき。\nとくぼうを さげることが ある。");

static const u8 sBraveBirdDescription[] = _("{JPN}ひくく とんで とつげき。\nじぶんも ダメージを うける。");

static const u8 sEarthPowerDescription[] = _("{JPN}じめんを ふんしゅつさせる。\nとくぼうを さげることが ある。");

static const u8 sSwitcherooDescription[] = _("{JPN}めにも とまらぬ はやさで\nあいてと どうぐを いれかえる。");

static const u8 sNastyPlotDescription[] = _("{JPN}わるい ことを かんがえて\nとくこうを ぐーんと あげる。");

static const u8 sBulletPunchDescription[] = _("{JPN}だんがんのような はやさの\nパンチ。かならず さきに あたる。");

static const u8 sIceShardDescription[] = _("{JPN}こおりの かたまりを なげる。\nかならず さきに あたる。");

static const u8 sShadowClawDescription[] = _("{JPN}かげの つめで こうげき。\nきゅうしょに あたりやすい。");

static const u8 sThunderFangDescription[] = _("{JPN}ひるませたり まひさせたり\nすることが ある。");

static const u8 sIceFangDescription[] = _("{JPN}ひるませたり こおらせたり\nすることが ある。");

static const u8 sFireFangDescription[] = _("{JPN}ひるませたり やけどさせたり\nすることが ある。");

static const u8 sShadowSneakDescription[] = _("{JPN}かげを のばして\nさきに こうげきする。");

static const u8 sMudBombDescription[] = _("{JPN}どろを なげつけて\nめいちゅうりつも さげる。");

static const u8 sPsychoCutDescription[] = _("{JPN}サイコの やいばで きりさく。\nきゅうしょに あたりやすい。");

static const u8 sZenHeadbuttDescription[] = _("{JPN}つよい ずつきで こうげき。\nひるませることが ある。");

static const u8 sMirrorShotDescription[] = _("{JPN}エネルギーの ひかりで こうげき。\nめいちゅうりつも さげる。");

static const u8 sFlashCannonDescription[] = _("{JPN}ひかりを はなって こうげき。\nとくぼうを さげることが ある。");

static const u8 sRockClimbDescription[] = _("{JPN}とつげきして こうげき。\nこんらんさせることが ある。");

static const u8 sDefogDescription[] = _("{JPN}じゃまな ものを とりのぞき\nかいひりつを さげる。");

static const u8 sTrickRoomDescription[] = _("{JPN}5ターンの あいだ おそい ほうが\nさきに うごくように なる。");

static const u8 sDracoMeteorDescription[] = _("{JPN}すいせいを ふらせて こうげき。\nとくこうが がくっと さがる。");

static const u8 sDischargeDescription[] = _("{JPN}でんげきで こうげき。\nまひさせることが ある。");

static const u8 sPowerWhipDescription[] = _("{JPN}つるや しょくしゅで はげしく\nうちつけて こうげき。");

static const u8 sCrossPoisonDescription[] = _("{JPN}きりさく こうげき。どくに したり\nきゅうしょに あたりやすい。");

static const u8 sGunkShotDescription[] = _("{JPN}きたない ゴミを なげつけて\nこうげき。 どくに することが ある。");

static const u8 sIronHeadDescription[] = _("{JPN}かたい あたまで こうげき。\nひるませることが ある。");

static const u8 sMagnetBombDescription[] = _("{JPN}じしゃくを はなって\nかならず あてる。");

static const u8 sStoneEdgeDescription[] = _("{JPN}いしで つきさして こうげき。\nきゅうしょに あたりやすい。");

static const u8 sCaptivateDescription[] = _("{JPN}せいべつが ちがう あいての\nとくこうを ぐーんと さげる。");

static const u8 sStealthRockDescription[] = _("{JPN}うかんだ いわを しかけて\nでてきた あいてに ダメージ。");

static const u8 sGrassKnotDescription[] = _("{JPN}あいてが おもいほど\nダメージが おおきく なる。");

static const u8 sChatterDescription[] = _("{JPN}おとの こうげき。\nこんらんさせることが ある。");

static const u8 sJudgmentDescription[] = _("{JPN}もっている プレートで\nタイプが かわる。");

static const u8 sChargeBeamDescription[] = _("{JPN}でんきの ビームで こうげき。\nとくこうが あがることが ある。");

static const u8 sWoodHammerDescription[] = _("{JPN}からだごと ぶつかって こうげき。\nじぶんも ダメージを うける。");

static const u8 sAquaJetDescription[] = _("{JPN}こうそくで つっこんで\nかならず さきに こうげき。");

static const u8 sAttackOrderDescription[] = _("{JPN}てしたが よってたかって こうげき。\nきゅうしょに あたりやすい。");

static const u8 sDefendOrderDescription[] = _("{JPN}いきものの たてで ぼうぎょと\nとくぼうを あげる。");

static const u8 sHealOrderDescription[] = _("{JPN}てしたが あらわれて さいだいHPの\nはんぶんを かいふくする。");

static const u8 sHeadSmashDescription[] = _("{JPN}いのちがけの ずつき。\nじぶんも おおきな ダメージ。");

static const u8 sDoubleHitDescription[] = _("{JPN}しっぽなどで 2かい\nうちつけて こうげき。");

static const u8 sRoarOfTimeDescription[] = _("{JPN}いりょくは たかいが つぎの\nターンは うごけなく なる。");

static const u8 sSpacialRendDescription[] = _("{JPN}あいてと くうかんを ひきさく。\nきゅうしょに あたりやすい。");

static const u8 sMagmaStormDescription[] = _(
    "Traps the foe in a vortex\n"
    "of fire for "BINDING_TURNS" turns.");

static const u8 sDarkVoidDescription[] = _("{JPN}まっくらやみに ひきずりこんで\nねむらせる。");

static const u8 sSeedFlareDescription[] = _("{JPN}しょうげきはで こうげき。\nとくぼうが ぐんと さがる。");

static const u8 sOminousWindDescription[] = _("{JPN}きもちわるい こうげき。 すべての\nのうりょくが あがることが ある。");

static const u8 sShadowForceDescription[] = _("{JPN}1ターンめに すがたを けし\n2ターンめに こうげきする。");

static const u8 sHoneClawsDescription[] = _("{JPN}つめを とぎ こうげきと\nめいちゅうりつを あげる。");

static const u8 sWideGuardDescription[] = _("{JPN}1ターンの あいだ ひろはんいの\nこうげきを かわす。");

static const u8 sGuardSplitDescription[] = _("{JPN}ぼうぎょと とくぼうの へんかを\nあいてと ならす。");

static const u8 sPowerSplitDescription[] = _("{JPN}こうげきと とくこうの へんかを\nあいてと ならす。");

static const u8 sWonderRoomDescription[] = _("{JPN}5ターンの あいだ ぼうぎょと\nとくぼうが いれかわる。");

static const u8 sPsyshockDescription[] = _("{JPN}サイコの なみで こうげき。\nぶつりの ダメージに なる。");

static const u8 sTailSlapDescription[] = _("{JPN}しっぽで 2から 5かい\nうちつけて こうげき。");

static const u8 sVenoshockDescription[] = _("{JPN}あいてが どくの ときは\nダメージが 2ばいに なる。");

static const u8 sAutotomizeDescription[] = _("{JPN}おもさを へらして\nすばやさを ぐーんと あげる。");

static const u8 sRagePowderDescription[] = _("{JPN}こなを まいて あいての こうげきを\nじぶんに あつめる。");

static const u8 sTelekinesisDescription[] = _("{JPN}あいてを うかせて 3ターンの\nあいだ あてやすく する。");

static const u8 sMagicRoomDescription[] = _("{JPN}5ターンの あいだ もちものの\nこうかが なくなる。");

static const u8 sSmackDownDescription[] = _("{JPN}いわを なげて あいてを\nじめんに おとす。");

static const u8 sStormThrowDescription[] = _("{JPN}かならず きゅうしょに あたる\nこうげき。");

static const u8 sFlameBurstDescription[] = _("{JPN}はじける ほのおで まわりの\nあいて ぜんいんに ダメージ。");

static const u8 sSludgeWaveDescription[] = _("{JPN}ヘドロの なみで こうげき。\nどくに することが ある。");

static const u8 sQuiverDanceDescription[] = _("{JPN}おどって とくこう とくぼう\nすばやさを あげる。");

static const u8 sHeavySlamDescription[] = _("{JPN}じぶんが あいてより おもいほど\nダメージが おおきく なる。");

static const u8 sSynchronoiseDescription[] = _("{JPN}ふしぎな しょうげきは。おなじ\nタイプの あいてだけに ダメージ。");

static const u8 sElectroBallDescription[] = _("{JPN}たまを なげる。あいてが おそいほど\nダメージが おおきい。");

static const u8 sSoakDescription[] = _("{JPN}みずを あびせて あいてを\nみずタイプに する。");

static const u8 sFlameChargeDescription[] = _("{JPN}ほのおを まとって こうげき。\nすばやさが あがる。");

static const u8 sCoilDescription[] = _("{JPN}とぐろを まいて こうげき ぼうぎょ\nめいちゅうりつを あげる。");

static const u8 sLowSweepDescription[] = _("{JPN}あいての あしを こうげきして\nすばやさを さげる。");

static const u8 sAcidSprayDescription[] = _("{JPN}かわを とかす さんを あびせる。\nとくぼうが ぐーんと さがる。");

static const u8 sFoulPlayDescription[] = _("{JPN}あいての こうげきが たかいほど\nダメージが おおきく なる。");

static const u8 sSimpleBeamDescription[] = _("{JPN}あいての とくせいを たんじゅんに\nかえる ビーム。");

static const u8 sEntrainmentDescription[] = _("{JPN}あいてを じぶんと おなじ\nとくせいに する。");

static const u8 sAfterYouDescription[] = _("{JPN}あいてを てだすけして\nつぎに うごかせる。");

static const u8 sRoundDescription[] = _("{JPN}ダメージを あたえる うた。\nなかまも いっしょに うたえる。");

static const u8 sEchoedVoiceDescription[] = _("{JPN}つかう ターンが つづくほど\nダメージが おおきく なる。");

static const u8 sChipAwayDescription[] = _("{JPN}あいての のうりょくへんかを\nむしして こうげきする。");

static const u8 sClearSmogDescription[] = _("{JPN}しろい きりで こうげきし すべての\nのうりょくへんかを もとに もどす。");

static const u8 sStoredPowerDescription[] = _("{JPN}じぶんの のうりょくが たかいほど\nダメージが おおきく なる。");

static const u8 sQuickGuardDescription[] = _("{JPN}1ターンの あいだ せんせいの\nこうげきを かわす。");

static const u8 sAllySwitchDescription[] = _("{JPN}なかまと ばしょを\nいれかえる。");

static const u8 sScaldDescription[] = _("{JPN}ねっとうを あびせて こうげき。\nやけどに することが ある。");

static const u8 sShellSmashDescription[] = _("{JPN}こうげきめんが あがるが\nぼうぎょめんが さがる。");

static const u8 sHealPulseDescription[] = _("{JPN}あいての さいだいHPの\nはんぶんまで かいふくする。");

static const u8 sHexDescription[] = _("{JPN}あいてが じょうたい いじょうなら\nダメージが 2ばいに なる。");

static const u8 sSkyDropDescription[] = _("{JPN}あいてを そらへ つれさり\nつぎの ターンに おとす。");

static const u8 sShiftGearDescription[] = _("{JPN}ギアを まわして こうげきと\nすばやさを あげる。");

static const u8 sCircleThrowDescription[] = _("{JPN}あいてを ふきとばして\nバトルを おわらせる。");

static const u8 sIncinerateDescription[] = _("{JPN}きのみや ジュエルを やきつくし\nつかえなく する。");

static const u8 sQuashDescription[] = _("{JPN}あいてを おさえこんで\nさいごに うごかせる。");

static const u8 sAcrobaticsDescription[] = _("{JPN}もちものが ないと\nダメージが 2ばいに なる。");

static const u8 sReflectTypeDescription[] = _("{JPN}あいての タイプを うつしとって\nおなじ タイプに なる。");

static const u8 sRetaliateDescription[] = _("{JPN}なかまが たおれていると\nダメージが おおきく なる。");

static const u8 sFinalGambitDescription[] = _("{JPN}じぶんが たおれて HPと おなじ\nダメージを あいてに あたえる。");

static const u8 sBestowDescription[] = _("{JPN}もちものを あいてに\nわたす。");

static const u8 sInfernoDescription[] = _("{JPN}つよいが あたりにくい。 かならず\nやけどに する。");

static const u8 sWaterPledgeDescription[] = _("{JPN}みずの はしらで こうげき。 にじが\nかかることが ある。");

static const u8 sFirePledgeDescription[] = _("{JPN}ほのおの はしらで こうげき。 くさを\nやくことが ある。");

static const u8 sGrassPledgeDescription[] = _("{JPN}くさの はしらで こうげき。 ぬまが\nできることが ある。");

static const u8 sStruggleBugDescription[] = _("{JPN}たえてから こうげき。 あいての\nとくこうを さげる。");

static const u8 sBulldozeDescription[] = _("{JPN}じめんを ふみつける。 あいての\nすばやさを さげる。");

static const u8 sWorkUpDescription[] = _("{JPN}きあいを たかめて こうげきと\nとくこうを あげる。");

static const u8 sElectrowebDescription[] = _("{JPN}でんきの あみで とらえ すばやさを\nさげる。");

static const u8 sWildChargeDescription[] = _("{JPN}でんきの たいあたり。 じぶんも\nダメージを うける。");

static const u8 sDrillRunDescription[] = _("{JPN}ドリルのように まわる。 きゅうしょに\nあたりやすい。");

static const u8 sDualChopDescription[] = _("{JPN}はげしい こうげきが 2かい\nあたる。");

static const u8 sHeartStampDescription[] = _("{JPN}かわいく してから ふいうち。\nひるませる ことが ある。");

static const u8 sRazorShellDescription[] = _("{JPN}するどい からで きりさく。\nぼうぎょを さげることが ある。");

static const u8 sLeafTornadoDescription[] = _("{JPN}はっぱで とりかこみ めいちゅうりつを\nさげる。");

static const u8 sSteamrollerDescription[] = _("{JPN}からだで おしつぶす。 ひるませる\nことが ある。");

static const u8 sCottonGuardDescription[] = _("{JPN}わたで つつみ ぼうぎょを ぐーんと\nあげる。");

static const u8 sNightDazeDescription[] = _("{JPN}まっくろな しょうげきは。\nめいちゅうを さげることが ある。");

static const u8 sHurricaneDescription[] = _("{JPN}はげしい かぜで とらえる。 こんらん\nさせることが ある。");

static const u8 sHeadChargeDescription[] = _("{JPN}かたい けで つっこむ。 じぶんも\nすこし きずつく。");

static const u8 sGearGrindDescription[] = _("{JPN}2つの はぐるまを なげ 2かい\nこうげきする。");

static const u8 sTechnoBlastDescription[] = _("{JPN}もっている カセットで タイプが\nかわる。");

static const u8 sRelicSongDescription[] = _("{JPN}いにしえの うた。 ねむらせる ことが\nある。");

static const u8 sSecretSwordDescription[] = _("{JPN}ながい つので きりつける ぶつりの\nこうげき。");

static const u8 sGlaciateDescription[] = _("{JPN}つめたい かぜを あてる。 すばやさを\nさげる。");

static const u8 sBoltStrikeDescription[] = _("{JPN}たいりょうの かみなり。 まひに\nすることが ある。");

static const u8 sBlueFlareDescription[] = _("{JPN}あおい ほのおで つつむ。 やけどに\nすることが ある。");

static const u8 sFieryDanceDescription[] = _("{JPN}ほのおを まとって おどる。\nとくこうが あがることが ある。");

static const u8 sFreezeShockDescription[] = _("{JPN}つよい 2ターンわざ。 まひに\nすることが ある。");

static const u8 sIceBurnDescription[] = _("{JPN}つよい 2ターンわざ。 やけどに\nすることが ある。");

static const u8 sSnarlDescription[] = _("{JPN}どなりつけて あいての とくこうを\nさげる。");

static const u8 sIcicleCrashDescription[] = _("{JPN}おおきな つららを おとす。\nひるませる ことが ある。");

static const u8 sVCreateDescription[] = _("{JPN}つよいが ぼうぎょ とくぼう\nすばやさが さがる。");

static const u8 sFusionFlareDescription[] = _("{JPN}ひのたまを よぶ。 かみなりの わざと\nあわせると つよい。");

static const u8 sFusionBoltDescription[] = _("{JPN}かみなりを よぶ。 ほのおの わざと\nあわせると つよい。");

static const u8 sFlyingPressDescription[] = _("{JPN}かくとうと ひこうの りょうほうで\nダメージを あたえる。");

static const u8 sMatBlockDescription[] = _("{JPN}1ターンの あいだ ダメージわざを\nかわす。");

static const u8 sBelchDescription[] = _("{JPN}おおきな ゲップ。 きのみを\nたべないと つかえない。");

static const u8 sRototillerDescription[] = _("{JPN}くさタイプの こうげきと とくこうを\nあげる。");

static const u8 sStickyWebDescription[] = _("{JPN}ネバネバの あみで でてきた あいての\nすばやさを さげる。");

static const u8 sFellStingerDescription[] = _("{JPN}あいてを たおすと じぶんの\nこうげきが あがる。");

static const u8 sTrickOrTreatDescription[] = _("{JPN}おかしを ねだって あいてを\nゴーストタイプに する。");

static const u8 sNobleRoarDescription[] = _("{JPN}おどして あいての こうげきと\nとくこうを さげる。");

static const u8 sIonDelugeDescription[] = _("{JPN}ノーマルの わざを でんきタイプに\nかえる。");

static const u8 sParabolicChargeDescription[] = _("{JPN}まわりに ダメージを あたえ\nそのはんぶん かいふく。");

static const u8 sForestsCurseDescription[] = _("{JPN}のろいを かけて あいてを\nくさタイプに する。");

static const u8 sPetalBlizzardDescription[] = _("{JPN}はげしい はなびらの あらしで\nこうげきする。");

static const u8 sFreezeDryDescription[] = _("{JPN}みずタイプに ばつぐん。 こおりに\nすることが ある。");

static const u8 sDisarmingVoiceDescription[] = _("{JPN}かわいい なきごえ。 かわされる\nことが ない。");

static const u8 sPartingShotDescription[] = _("{JPN}こうげきと とくこうを さげてから\nいれかわる。");

static const u8 sTopsyTurvyDescription[] = _("{JPN}あいての のうりょくへんかを\nいれかえる。");

static const u8 sDrainingKissDescription[] = _("{JPN}あたえた ダメージの はんぶん\nいじょうを すいとる。");

static const u8 sCraftyShieldDescription[] = _("{JPN}1ターンの あいだ へんかわざを\nかわす。");

static const u8 sFlowerShieldDescription[] = _("{JPN}くさタイプの ぼうぎょを\nあげる。");

static const u8 sGrassyTerrainDescription[] = _("{JPN}5ターン じめんが くさに なり\nHPが かいふく する。");

static const u8 sMistyTerrainDescription[] = _("{JPN}5ターン きりが かかり じょうたい\nいじょうを ふせぐ。");

static const u8 sElectrifyDescription[] = _("{JPN}あいての つぎの わざを\nでんきタイプに する。");

static const u8 sPlayRoughDescription[] = _("{JPN}あらっぽく あそぶ。 こうげきを\nさげることが ある。");

static const u8 sFairyWindDescription[] = _("{JPN}フェアリーの かぜで あいてを\nこうげきする。");

static const u8 sMoonblastDescription[] = _("{JPN}つきの ちからで こうげき。\nとくこうを さげることが ある。");

static const u8 sBoomburstDescription[] = _("{JPN}はかいてきな おんぱで まわり\nぜんぶを こうげき。");

static const u8 sFairyLockDescription[] = _("{JPN}つぎの ターン あいてを にげられなく\nする。");

static const u8 sKingsShieldDescription[] = _("{JPN}ダメージを かわし うけたら\nこうげきを がくっと さげる。");

static const u8 sPlayNiceDescription[] = _("{JPN}なかよく なって かならず こうげきを\nさげる。");

static const u8 sConfideDescription[] = _("{JPN}ひみつを うちあけて あいての\nとくこうを さげる。");

static const u8 sDiamondStormDescription[] = _("{JPN}ダイヤの あらしで こうげき。\nぼうぎょが あがることが ある。");

static const u8 sSteamEruptionDescription[] = _("{JPN}あつい じょうきで つつむ。 やけどに\nすることが ある。");

static const u8 sHyperspaceHoleDescription[] = _("{JPN}ゆがみを つかって こうげき。\nかわされない。");

static const u8 sWaterShurikenDescription[] = _("{JPN}ほしを 2ー5こ なげる。 かならず\nせんせい こうげき。");

static const u8 sMysticalFireDescription[] = _("{JPN}とくべつな あつい ほのお。\nとくこうを さげる。");

static const u8 sSpikyShieldDescription[] = _("{JPN}こうげきを かわし うけたら あいてに\nダメージ。");

static const u8 sAromaticMistDescription[] = _("{JPN}みかたの とくぼうを\nあげる。");

static const u8 sEerieImpulseDescription[] = _("{JPN}はどうを あてて とくこうを がくっと\nさげる。");

static const u8 sVenomDrenchDescription[] = _("{JPN}どくの あいての こうげき とくこう\nすばやさを さげる。");

static const u8 sPowderDescription[] = _("{JPN}あいてが ほのおの わざを つかうと\nダメージを あたえる。");

static const u8 sGeomancyDescription[] = _("{JPN}2ターンめに とくこう とくぼう\nすばやさが あがる。");

static const u8 sMagneticFluxDescription[] = _("{JPN}プラスか マイナスを もつ ものの\nまもりを あげる。");

static const u8 sHappyHourDescription[] = _("{JPN}もらえる しょうきんが 2ばいに\nなる。");

static const u8 sElectricTerrainDescription[] = _("{JPN}5ターン じめんが でんきに なり\nねむらなく なる。");

static const u8 sDazzlingGleamDescription[] = _("{JPN}まばゆい ひかりを だして あいてに\nダメージ。");

static const u8 sCelebrateDescription[] = _("{JPN}とくべつな ひを おいわい\nしてくれる。");

static const u8 sHoldHandsDescription[] = _("{JPN}みかたと てを つないで しあわせな\nきもちに なる。");

static const u8 sBabyDollEyesDescription[] = _("{JPN}あいてが うごく まえに こうげきを\nさげる。");

static const u8 sNuzzleDescription[] = _("{JPN}ほっぺたを こすりつけて あいてを\nまひに する。");

static const u8 sInfestationDescription[] = _(
    "The foe is infested and\n"
    "attacked for "BINDING_TURNS" turns.");

static const u8 sPowerUpPunchDescription[] = _("{JPN}かたい パンチ。 じぶんの こうげきが\nあがる。");

static const u8 sThousandArrowsDescription[] = _("{JPN}そらに いる あいてにも あたり\nじめんに おとす。");

static const u8 sThousandWavesDescription[] = _("{JPN}はどうを うけた あいては\nにげられなく なる。");

static const u8 sLandsWrathDescription[] = _("{JPN}だいちの ちからを あつめて あいて\nぜんいんを こうげき。");

static const u8 sLightOfRuinDescription[] = _("{JPN}おおきな ひかりの ビーム。 じぶんも\nダメージを うける。");

static const u8 sOriginPulseDescription[] = _("{JPN}あおく ひかる こうせんが あいて\n2ひきを おそう。");

static const u8 sPrecipiceBladesDescription[] = _("{JPN}おそろしい いわの やいばが あいて\n2ひきを おそう。");

static const u8 sLavaPlumeDescription[] = _("{JPN}まっかな ほのおが まわり ぜんぶを\nやきつくす。");

static const u8 sLeafStormDescription[] = _("{JPN}はっぱの あらし。 とくこうを\nがくっと さげる。");

static const u8 sShoreUpDescription[] = _("{JPN}HPを かいふく。 すなあらしなら\nおおく かいふく。");

static const u8 sFirstImpressionDescription[] = _("{JPN}つよく せんせい こうげき。 でた\nターンだけ つかえる。");

static const u8 sBanefulBunkerDescription[] = _("{JPN}じぶんを まもり ふれた あいてを\nどくに する。");

static const u8 sSpiritShackleDescription[] = _("{JPN}こうげきを うけた あいては\nにげられなく なる。");

static const u8 sDarkestLariatDescription[] = _("{JPN}うでを ふって こうげき。\nのうりょくへんかを むしする。");

static const u8 sSparklingAriaDescription[] = _("{JPN}あわの うた。 ふれた あいての\nやけどを なおす。");

static const u8 sIceHammerDescription[] = _("{JPN}こぶしで なぐる。 じぶんの\nすばやさが さがる。");

static const u8 sFloralHealingDescription[] = _("{JPN}みかたの HPを かいふく。 くさの\nうえなら おおく かいふく。");

static const u8 sHighHorsepowerDescription[] = _("{JPN}ぜんしんで あいてに はげしく\nぶつかる。");

static const u8 sStrengthSapDescription[] = _("{JPN}あいての こうげきを すいとり HPを\nかいふく する。");

static const u8 sSolarBladeDescription[] = _("{JPN}1ターンめに ためて ひかりの\nやいばで きりつける。");

static const u8 sLeafageDescription[] = _("{JPN}ちいさな はっぱを たくさん とばして\nこうげき。");

static const u8 sSpotlightDescription[] = _("{JPN}あいてを めだつ ポケモンに こうげき\nさせる。");

static const u8 sToxicThreadDescription[] = _("{JPN}いとで こうげき。 どくに して\nすばやさを さげる。");

static const u8 sLaserFocusDescription[] = _("{JPN}つぎの わざが かならず きゅうしょに\nあたる。");

static const u8 sGearUpDescription[] = _("{JPN}プラスか マイナスを もつ ものの\nこうげきを あげる。");

static const u8 sThroatChopDescription[] = _("{JPN}のどを ついて しばらく おとの\nわざを ふうじる。");

static const u8 sPollenPuffDescription[] = _("{JPN}あいてに ばくはつ するが みかたの\nHPを かいふく。");

static const u8 sAnchorShotDescription[] = _("{JPN}くさりで しめつける。 あいては\nにげられなく なる。");

static const u8 sPsychicTerrainDescription[] = _("{JPN}5ターン じめんが かわり さきどりの\nわざを ふせぐ。");

static const u8 sLungeDescription[] = _("{JPN}とびかかって あいての こうげきを\nさげる。");

static const u8 sFireLashDescription[] = _("{JPN}ほのおの ムチで うつ。 あいての\nぼうぎょを さげる。");

static const u8 sPowerTripDescription[] = _("{JPN}のうりょくが あがっているほど\nいりょくが あがる。");

static const u8 sBurnUpDescription[] = _("{JPN}もえつきて じぶんの ほのおタイプが\nなくなる。");

static const u8 sSpeedSwapDescription[] = _("{JPN}じぶんの すばやさを あいてと\nいれかえる。");

static const u8 sSmartStrikeDescription[] = _("{JPN}せいかくな つので つく。 かならず\nめいちゅう する。");

static const u8 sPurifyDescription[] = _("{JPN}あいての いじょうを なおして HPを\nかいふく する。");

static const u8 sRevelationDanceDescription[] = _("{JPN}しんぴの まい。 じぶんの さいしょの\nタイプに なる。");

static const u8 sCoreEnforcerDescription[] = _("{JPN}こうせんで こうげきし あいての\nとくせいを けす。");

static const u8 sTropKickDescription[] = _("{JPN}みなみの はげしい けり。 こうげきを\nさげる。");

static const u8 sInstructDescription[] = _("{JPN}あいてに さいごの わざを もういちど\nつかわせる。");

static const u8 sBeakBlastDescription[] = _("{JPN}くちばしを あつくして こうげき。\nあいてを やけどに する。");

static const u8 sClangingScalesDescription[] = _("{JPN}うろこで おおきな おと。 ぼうぎょを\nさげる。");

static const u8 sDragonHammerDescription[] = _("{JPN}からだを ハンマーのように ふって\nダメージを あたえる。");

static const u8 sBrutalSwingDescription[] = _("{JPN}はげしく あばれて まわりの もの\nぜんぶを きずつける。");

static const u8 sAuroraVeilDescription[] = _("{JPN}すべての こうげきを よわめる。\nあられの ときだけ。");

static const u8 sShellTrapDescription[] = _("{JPN}からの わなを しかけ ふれた\nあいてに ダメージ。");

static const u8 sFleurCannonDescription[] = _("{JPN}つよい こうせん。 とくこうを\nがくっと さげる。");

static const u8 sPsychicFangsDescription[] = _("{JPN}ねんりきの キバで かむ。 かべを\nこわす。");

static const u8 sStompingTantrumDescription[] = _("{JPN}おこって あばれる。 しっぱいの\nあとは つよく なる。");

static const u8 sShadowBoneDescription[] = _("{JPN}のろいの ホネで たたく。 ぼうぎょを\nさげることが ある。");

static const u8 sAccelerockDescription[] = _("{JPN}こうそくの いわ。 かならず せんせい\nこうげき できる。");

static const u8 sLiquidationDescription[] = _("{JPN}みずを たたきつける。 ぼうぎょを\nさげることが ある。");

static const u8 sPrismaticLaserDescription[] = _("{JPN}つよい レーザー。 つぎの ターンは\nうごけない。");

static const u8 sSpectralThiefDescription[] = _("{JPN}あいての のうりょくアップを\nうばってから こうげき。");

static const u8 sSunsteelStrikeDescription[] = _("{JPN}たいようの ちからの こうげき。\nとくせいを むしする。");

static const u8 sMoongeistBeamDescription[] = _("{JPN}つきの ちからの ビーム。 とくせいを\nむしする。");

static const u8 sTearfulLookDescription[] = _("{JPN}なみだを ながし こうげきと\nとくこうを さげる。");

static const u8 sZingZapDescription[] = _("{JPN}でんきの しょうげき。 ひるませる\nことが ある。");

static const u8 sNaturesMadnessDescription[] = _("{JPN}しぜんの ちからで あいての HPを\nはんぶんに する。");

static const u8 sMultiAttackDescription[] = _("{JPN}メモリに よって かわる\nこうげき。");

static const u8 sMindBlownDescription[] = _("{JPN}あたまを ばくはつ させて まわり\nぜんぶに ダメージ。");

static const u8 sPlasmaFistsDescription[] = _("{JPN}でんきの こぶし。 ノーマルわざが\nでんきタイプに なる。");

static const u8 sPhotonGeyserDescription[] = _("{JPN}こうげきと とくこうの たかい ほうで\nこうげきする。");

static const u8 sZippyZapDescription[] = _(
    "Electric bursts always go\n"
#if B_UPDATED_MOVE_DATA >= GEN_8
    "first and raises evasiveness.");
#else
    "first and land a critical hit.");
#endif

static const u8 sSplishySplashDescription[] = _("{JPN}おおきな でんきの なみ。 まひに\nすることが ある。");

static const u8 sFloatyFallDescription[] = _("{JPN}うかんでから ななめに おそう。\nひるませる ことが ある。");

static const u8 sPikaPapowDescription[] = _("{JPN}ピカチュウの あいじょうで\nつよくなる。 かならず あたる。");

static const u8 sBouncyBubbleDescription[] = _(
    "An attack that absorbs\n"
#if B_UPDATED_MOVE_DATA >= GEN_8
    "all the damage inflicted.");
#else
    "half the damage inflicted.");
#endif

static const u8 sBuzzyBuzzDescription[] = _("{JPN}でんげきを はなつ。 かならず\nまひじょうたいに する。");

static const u8 sSizzlySlideDescription[] = _("{JPN}ほのおを まとって つっこむ。\nあいてを やけどに する。");

static const u8 sGlitzyGlowDescription[] = _("{JPN}ねんりきの かべを つくり とくしゅの\nダメージを よわめる。");

static const u8 sBaddyBadDescription[] = _("{JPN}わるさを して こうげきし ぶつりの\nダメージを よわめる。");

static const u8 sSappySeedDescription[] = _("{JPN}おおきな くきの タネが まいターン\nHPを すいとる。");

static const u8 sFreezyFrostDescription[] = _("{JPN}つめたい けっしょうが おそい\nのうりょくへんかを けす。");

static const u8 sSparklySwirlDescription[] = _("{JPN}かおりの うずで つつみ みかたの\nいじょうを なおす。");

static const u8 sVeeveeVolleyDescription[] = _("{JPN}イーブイの あいじょうで つよくなる。\nかならず あたる。");

static const u8 sDoubleIronBashDescription[] = _("{JPN}かいてん して うでで たたく。\nひるませる ことが ある。");

// GEN 8
static const u8 sDynamaxCannonDescription[] = _("{JPN}つよい ビーム。 ダイマックスの\nあいてに ばいの ダメージ。");

static const u8 sSnipeShotDescription[] = _("{JPN}わざを ひきよせる こうかを\nむしする。");

static const u8 sJawLockDescription[] = _("{JPN}じぶんも あいても にげられなく\nなる。");

static const u8 sStuffCheeksDescription[] = _("{JPN}きのみを たべて ぼうぎょを ぐーんと\nあげる。");

static const u8 sNoRetreatDescription[] = _("{JPN}すべての のうりょくが あがるが\nにげられなく なる。");

static const u8 sTarShotDescription[] = _("{JPN}あいての すばやさを さげ ほのおに\nよわく する。");

static const u8 sMagicPowderDescription[] = _("{JPN}ふしぎな こなで あいてを\nエスパータイプに する。");

static const u8 sDragonDartsDescription[] = _("{JPN}2かい こうげき。 あいてが\n2ひきなら 1かいずつ。");

static const u8 sTeatimeDescription[] = _("{JPN}みんなで おちゃに して きのみを\nたべる。");

static const u8 sOctolockDescription[] = _("{JPN}とじこめて まいターン ぼうぎょと\nとくぼうを さげる。");

static const u8 sBoltBeakDescription[] = _("{JPN}あいてより さきに うごくと\nいりょくが ばいに なる。");

static const u8 sFishiousRendDescription[] = _("{JPN}あいてより さきに うごくと\nいりょくが ばいに なる。");

static const u8 sCourtChangeDescription[] = _("{JPN}じぶんがわと あいてがわの こうかを\nいれかえる。");

static const u8 sClangorousSoulDescription[] = _("{JPN}HPを つかって すべての\nのうりょくを あげる。");

static const u8 sBodyPressDescription[] = _("{JPN}じぶんの ぼうぎょが たかいほど\nダメージが おおきい。");

static const u8 sDecorateDescription[] = _("{JPN}あいての こうげきと とくこうを\nぐーんと あげる。");

static const u8 sDrumBeatingDescription[] = _("{JPN}たいこを たたいて こうげき。\nすばやさを さげる。");

static const u8 sSnapTrapDescription[] = _("{JPN}わなで 4ー5ターン とじこめて\nこうげきする。");

static const u8 sPyroBallDescription[] = _("{JPN}ほのおの たまを なげる。 やけどに\nすることが ある。");

static const u8 sBehemothBladeDescription[] = _("{JPN}つるぎで きる。 ダイマックスの\nあいてに ばいの ダメージ。");

static const u8 sBehemothBashDescription[] = _("{JPN}たてで こうげき。 ダイマックスの\nあいてに ばいの ダメージ。");

static const u8 sAuraWheelDescription[] = _("{JPN}すばやさを あげて こうげき。\nすがたで タイプが かわる。");

static const u8 sBreakingSwipeDescription[] = _("{JPN}しっぽで たたく。 あたった あいての\nこうげきを さげる。");

static const u8 sBranchPokeDescription[] = _("{JPN}とがった えだで あいてを つついて\nこうげきする。");

static const u8 sOverdriveDescription[] = _("{JPN}ギターを かきならして つよい\nしんどうを おこす。");

static const u8 sAppleAcidDescription[] = _("{JPN}すっぱい しるで あいての とくぼうを\nさげる。");

static const u8 sGravAppleDescription[] = _("{JPN}うえから リンゴを おとす。\nぼうぎょを さげる。");

static const u8 sSpiritBreakDescription[] = _("{JPN}きあいを くじく こうげき。\nとくこうを さげる。");

static const u8 sStrangeSteamDescription[] = _("{JPN}あやしい じょうきで こんらん\nさせることが ある。");

static const u8 sLifeDewDescription[] = _("{JPN}みずを まいて じぶんと みかたの\nHPを かいふく。");

static const u8 sObstructDescription[] = _("{JPN}じぶんを まもり ふれた あいての\nぼうぎょを がくっと さげる。");

static const u8 sFalseSurrenderDescription[] = _("{JPN}おじぎ して けで つきさす。\nかならず めいちゅう。");

static const u8 sMeteorAssaultDescription[] = _("{JPN}ふとい ネギで こうげき。 つぎの\nターンは やすむ。");

static const u8 sEternabeamDescription[] = _("{JPN}ムゲンダイナの さいきょうの わざ。\nつぎは やすむ。");

static const u8 sSteelBeamDescription[] = _("{JPN}はがねの こうせん。 じぶんも\nダメージを うける。");

static const u8 sExpandingForceDescription[] = _("{JPN}サイコフィールドで つよくなり あいて\nぜんいんに あたる。");

static const u8 sSteelRollerDescription[] = _("{JPN}フィールドを こわす。 フィールドが\nないと しっぱい。");

static const u8 sScaleShotDescription[] = _("{JPN}うろこを 2ー5かい とばす。\nすばやさアップ ぼうぎょダウン。");

static const u8 sMeteorBeamDescription[] = _("{JPN}1ターンめに とくこうを あげ\n2ターンめに こうげき。");

static const u8 sShellSideArmDescription[] = _("{JPN}ぶつりと とくしゅの つよい ほうで\nこうげき。 どくに することも。");

static const u8 sMistyExplosionDescription[] = _("{JPN}まわりを こうげきして ひんしに\nなる。 ミストで つよい。");

static const u8 sGrassyGlideDescription[] = _("{JPN}じめんを すべって こうげき。\nグラスなら せんせい。");

static const u8 sRisingVoltageDescription[] = _("{JPN}エレキフィールドの とき いりょくが\nばいに なる。");

static const u8 sTerrainPulseDescription[] = _("{JPN}フィールドで タイプと いりょくが\nかわる。");

static const u8 sSkitterSmackDescription[] = _("{JPN}うしろに まわって こうげき。\nとくこうを さげる。");

static const u8 sBurningJealousyDescription[] = _("{JPN}その ターンに のうりょくが あがった\nあいてを やけどに する。");

static const u8 sLashOutDescription[] = _("{JPN}その ターンに のうりょくが さがると\nいりょくが ばいに。");

static const u8 sPoltergeistDescription[] = _("{JPN}あいての どうぐで こうげき。\nもっていないと しっぱい。");

static const u8 sCorrosiveGasDescription[] = _("{JPN}つよい さんの ガスで まわりの\nもちものを とかす。");

static const u8 sCoachingDescription[] = _("{JPN}みかたを はげまし こうげきと\nぼうぎょを あげる。");

static const u8 sFlipTurnDescription[] = _("{JPN}こうげきの あと ひかえの ポケモンと\nいれかわる。");

static const u8 sTripleAxelDescription[] = _("{JPN}3かいの けり。 あたる たびに\nいりょくが あがる。");

static const u8 sDualWingbeatDescription[] = _("{JPN}つばさで たたきつけ 2かい つづけて\nこうげきする。");

static const u8 sScorchingSandsDescription[] = _("{JPN}やけた すなを なげつける。 やけどに\nすることが ある。");

static const u8 sJungleHealingDescription[] = _("{JPN}じぶんと みかたの HPと\nじょうたいを かいふく する。");

static const u8 sWickedBlowDescription[] = _("{JPN}あくの かたを きわめた いちげき。\nかならず きゅうしょ。");

static const u8 sSurgingStrikesDescription[] = _("{JPN}みずの かたを きわめた 3れんげき。\nかならず きゅうしょ。");

static const u8 sThunderCageDescription[] = _(
    "Traps the foe in a cage of\n"
    "electricity for "BINDING_TURNS" turns.");

static const u8 sDragonEnergyDescription[] = _("{JPN}じぶんの HPが すくない ほど\nわざの いりょくは さがる。");

static const u8 sFreezingGlareDescription[] = _("{JPN}めから ねんりき。 こおりに\nすることが ある。");

static const u8 sFieryWrathDescription[] = _("{JPN}いかりの こうげき。 ひるませる\nことが ある。");

static const u8 sThunderousKickDescription[] = _("{JPN}かみなりのような けり。 ぼうぎょを\nさげる。");

static const u8 sGlacialLanceDescription[] = _("{JPN}ふぶきを まとった つららの やりで\nあいてを つらぬく。");

static const u8 sAstralBarrageDescription[] = _("{JPN}おそろしい かずの れいを あいてに\nおくりつける。");

static const u8 sEerieSpellDescription[] = _("{JPN}ねんりきの こうげき。 あいての\nさいごの わざの PPを 3 へらす。");

static const u8 sDireClawDescription[] = _("{JPN}きゅうしょに あたりやすい。 まひ\nどく ねむりに することも。");

static const u8 sPsyshieldBashDescription[] = _("{JPN}ねんりきで こうげき。 ぼうぎょが\nあがることが ある。");

static const u8 sPowerShiftDescription[] = _("{JPN}じぶんの こうげきと ぼうぎょを\nいれかえる。");

static const u8 sStoneAxeDescription[] = _("{JPN}きゅうしょに あたりやすく とげが\nあいてを きずつける。");

static const u8 sSpringtideStormDescription[] = _("{JPN}はげしい かぜで つつむ。 すがたで\nこうかが かわる。");

static const u8 sMysticalPowerDescription[] = _("{JPN}ふしぎな ちからの こうげき。\nじぶんの とくこうが あがる。");

static const u8 sRagingFuryDescription[] = _("{JPN}2から 3ターン あばれて\nそのあと こんらんする。");

static const u8 sWaveCrashDescription[] = _("{JPN}みずを まとった たいあたり。\nじぶんも きずつく。");

static const u8 sChloroblastDescription[] = _("{JPN}ようりょくそを あつめた こうげき。\nじぶんも きずつく。");

static const u8 sMountainGaleDescription[] = _("{JPN}おおきな こおりの かたまり。\nひるませる ことが ある。");

static const u8 sVictoryDanceDescription[] = _("{JPN}おどって こうげき ぼうぎょ\nすばやさを あげる。");

static const u8 sHeadlongRushDescription[] = _("{JPN}ぜんしんの たいあたり。 じぶんの\nまもりが さがる。");

static const u8 sBarbBarrageDescription[] = _("{JPN}どくに することが あり どくの\nあいてには つよい。");

static const u8 sEsperWingDescription[] = _("{JPN}きゅうしょに あたりやすい。 じぶんの\nすばやさが あがる。");

static const u8 sBitterMaliceDescription[] = _("{JPN}ぞっとする うらみ。 こうげきを\nさげることが ある。");

static const u8 sShelterDescription[] = _("{JPN}ひふを かたくして ぼうぎょを\nぐーんと あげる。");

static const u8 sTripleArrowsDescription[] = _("{JPN}きゅうしょに あたりやすい。\nぼうぎょダウンか ひるみも。");

static const u8 sInfernalParadeDescription[] = _("{JPN}いじょうの あいてに つよい。\nやけどに することが ある。");

static const u8 sCeaselessEdgeDescription[] = _("{JPN}きゅうしょに あたりやすく とげが\nあいてを きずつける。");

static const u8 sBleakwindStormDescription[] = _("{JPN}はげしい つめたい かぜ。 すばやさを\nさげることが ある。");

static const u8 sWildboltStormDescription[] = _("{JPN}はげしい あらし。 まひに することが\nある。");

static const u8 sSandsearStormDescription[] = _("{JPN}やけた すな。 やけどに することが\nある。");

static const u8 sLunarBlessingDescription[] = _("{JPN}じぶんと みかたの HPと\nじょうたいを なおす。");

static const u8 sTakeHeartDescription[] = _("{JPN}きあいを たかめて じぶんを\nかいふくし つよくする。");

static const u8 sTeraBlastDescription[] = _("{JPN}テラスタルして いると テラスタイプで\nこうげきする。");

static const u8 sSilkTrapDescription[] =_("{JPN}じぶんを まもり ふれた あいての\nすばやさを さげる。");

static const u8 sAxeKickDescription[] = _("{JPN}はずすと じぶんが きずつく。\nこんらん させることが ある。");

static const u8 sLastRespectsDescription[] = _("{JPN}たおれた みかたが おおいほど\nダメージが おおきい。");

static const u8 sLuminaCrashDescription[] = _("{JPN}こころに ひびく ひかり。 とくぼうを\nがくっと さげる。");

static const u8 sOrderUpDescription[] = _("{JPN}シャリタツに よって のうりょくが\nあがる。");

static const u8 sJetPunchDescription[] = _("{JPN}めにも とまらぬ パンチ。 かならず\nせんせい こうげき。");

static const u8 sSpicyExtractDescription[] = _("{JPN}あいての こうげきを ぐーんと あげ\nぼうぎょを がくっと さげる。");

static const u8 sSpinOutDescription[] = _("{JPN}あしに ちからを こめる。 じぶんの\nすばやさが がくっと さがる。");

static const u8 sPopulationBombDescription[] = _("{JPN}なかまが 1ー10かい つづけて\nこうげきする。");

static const u8 sIceSpinnerDescription[] = _("{JPN}こおった あしで けり フィールドを\nこわす。");

static const u8 sGlaiveRushDescription[] = _("{JPN}つぎの ターン あいての こうげきが\nかならず あたり ばいに なる。");

static const u8 sRevivalBlessingDescription[] = _("{JPN}ひんしの {PKMN}を いきかえらせ\nHPを はんぶん もどす。");

static const u8 sSaltCureDescription[] = _("{JPN}まいターン ダメージ。 はがねと\nみずには ばいの ダメージ。");

static const u8 sTripleDiveDescription[] = _("{JPN}みずしぶきで 3かい つづけて\nこうげきする。");

static const u8 sMortalSpinDescription[] = _("{JPN}わなや やどりぎを けし まわりの\nあいてを どくに する。");

static const u8 sDoodleDescription[] = _("{JPN}じぶんと みかたの とくせいを\nあいてと おなじに する。");

static const u8 sFilletAwayDescription[] = _("{JPN}HPを つかって こうげき とくこう\nすばやさを ぐーんと あげる。");

static const u8 sKowtowCleaveDescription[] = _("{JPN}おじぎ してから きりつける。\nかならず めいちゅう。");

static const u8 sFlowerTrickDescription[] = _("{JPN}しかけの はなたば。 かならず\nきゅうしょに あたる。");

static const u8 sTorchSongDescription[] = _("{JPN}ほのおで やきつくす。 じぶんの\nとくこうが あがる。");

static const u8 sAquaStepDescription[] = _("{JPN}かるい ステップで おどる。 じぶんの\nすばやさが あがる。");

static const u8 sRagingBullDescription[] = _("{JPN}かべを こわす たいあたり。 すがたで\nタイプが かわる。");

static const u8 sMakeItRainDescription[] = _("{JPN}じぶんの とくこうが さがる。 あとで\nおかねが もらえる。");

static const u8 sRuinationDescription[] = _("{JPN}わざわいを よび あいての HPを\nはんぶんに する。");

static const u8 sCollisionCourseDescription[] = _("{JPN}たいこの ばくはつ。\nこうかばつぐんなら もっと つよい。");

static const u8 sElectroDriftDescription[] = _("{JPN}みらいの でんき。 こうかばつぐんなら\nもっと つよい。");

static const u8 sShedTailDescription[] = _("{JPN}みがわりを だしてから ひかえと\nいれかわる。");

static const u8 sChillyReceptionDescription[] =_("{JPN}へたな じょうだんで ゆきを ふらせ\nじぶんは いれかわる。");

static const u8 sTidyUpDescription[] = _("{JPN}わなを かたづけ こうげきと\nすばやさを あげる。");

static const u8 sSnowscapeDescription[] = _("{JPN}5ターンの あいだ ゆきを\nふらせる。");

static const u8 sPounceDescription[] = _("{JPN}とびかかって あいての すばやさを\nさげる。");

static const u8 sTrailblazeDescription[] = _("{JPN}ふいに こうげきして じぶんの\nすばやさを あげる。");

static const u8 sChillingWaterDescription[] = _("{JPN}つめたい みずを あびせて あいての\nこうげきを さげる。");

static const u8 sHyperDrillDescription[] = _("{JPN}とがった ぶぶんが まわり あいての\nまもるを ぬける。");

static const u8 sTwinBeamDescription[] = _("{JPN}ふしぎな めからの こうせんが 2かい\nつづけて あたる。");

static const u8 sRageFistDescription[] = _("{JPN}こうげきを うけた かいすうが\nおおいほど つよく なる。");

static const u8 sArmorCannonDescription[] = _("{JPN}つよい こうげきだが\nぼうぎょめんが さがる。");

static const u8 sBitterBladeDescription[] = _("{JPN}あいてに あたえた ダメージの\nはんぶんの HPを かいふくできる。");

static const u8 sDoubleShockDescription[] = _("{JPN}でんきを ぜんぶ だしきり\nでんきタイプが なくなる。");

static const u8 sGigatonHammerDescription[] = _("{JPN}おおきな ハンマー。 2かい\nつづけては つかえない。");

static const u8 sComeuppanceDescription[] = _("{JPN}さいごに きずつけた あいてに つよく\nはんげき する。");

static const u8 sAquaCutterDescription[] = _("{JPN}あつを かけた みずの やいば。\nきゅうしょに あたりやすい。");

static const u8 sBlazingTorqueDescription[] = _(
    "---");

static const u8 sWickedTorqueDescription[] = _(
    "---");

static const u8 sNoxiousTorqueDescription[] = _(
    "---");

static const u8 sCombatTorqueDescription[] = _(
    "---");

static const u8 sMagicalTorqueDescription[] = _(
    "---");

static const u8 sPsybladeDescription[] = _("{JPN}エレキフィールドの とき いりょくが\nあがる。");

static const u8 sHydroSteamDescription[] = _("{JPN}ひざしが つよい とき いりょくが\nあがる。");

static const u8 sBloodMoonDescription[] = _("{JPN}ブラッドムーンを はなつ。 2かい\nつづけては つかえない。");

static const u8 sMatchaGotchaDescription[] = _("{JPN}あたえた ダメージの はんぶん\nすいとる。 やけどに することも。");

static const u8 sSyrupBombDescription[] = _("{JPN}3ターンの あいだ まいターン\nあいての すばやさを さげる。");

static const u8 sIvyCudgelDescription[] = _("{JPN}もつ めんで タイプが かわる。\nきゅうしょに あたりやすい。");

static const u8 sElectroShotDescription[] = _("{JPN}1ターンめに でんきを ためて\n2ターンめに こうげき。");

static const u8 sTeraStarstormDescription[] = _("{JPN}ステラの テラパゴスなら あいて\nぜんいんに ダメージ。");

static const u8 sFickleBeamDescription[] = _("{JPN}ひかりの ビーム。 ときどき\nいりょくが ばいに なる。");

static const u8 sBurningBulwarkDescription[] = _("{JPN}こうげきを かわし うけたら あいてを\nやけどに する。");

static const u8 sTachyonCutterDescription[] = _("{JPN}りゅうしの やいばを 2かい つづけて\nはなつ。");

static const u8 sDragonCheerDescription[] = _("{JPN}みかたが きゅうしょに あたりやすく\nなる。 ドラゴンなら さらに。");

static const u8 sAlluringVoiceDescription[] = _("{JPN}その ターンに のうりょくが あがった\nあいてを こんらんさせる。");

static const u8 sTemperFlareDescription[] = _("{JPN}すてみの こうげき。 まえの わざが\nしっぱいなら ばいに なる。");

static const u8 sSupercellSlamDescription[] = _("{JPN}でんきの たいあたり。 はずすと\nじぶんが きずつく。");

static const u8 sPsychicNoiseDescription[] = _("{JPN}いやな おんぱで ダメージを あたえ\nかいふくを ふうじる。");

static const u8 sUpperHandDescription[] = _("{JPN}さきどりの わざを だそうと する\nあいてを ひるませる。");

static const u8 sMalignantChainDescription[] = _("{JPN}とかす くさりの こうげき。\nもうどくに することが ある。");

const u8 gNotDoneYetDescription[] = _("{JPN}この わざは つかえません。 こうかは\nかいはつちゅうです。");

#undef BINDING_TURNS

// MOVE_NONE is ignored in this table. Make sure to always subtract 1 before getting the right pointer.
const u8 *const gMoveDescriptionPointers[MOVES_COUNT - 1] =
{
    [MOVE_POUND - 1] = sPoundDescription,
    [MOVE_KARATE_CHOP - 1] = sKarateChopDescription,
    [MOVE_DOUBLE_SLAP - 1] = sDoubleSlapDescription,
    [MOVE_COMET_PUNCH - 1] = sCometPunchDescription,
    [MOVE_MEGA_PUNCH - 1] = sMegaPunchDescription,
    [MOVE_PAY_DAY - 1] = sPayDayDescription,
    [MOVE_FIRE_PUNCH - 1] = sFirePunchDescription,
    [MOVE_ICE_PUNCH - 1] = sIcePunchDescription,
    [MOVE_THUNDER_PUNCH - 1] = sThunderPunchDescription,
    [MOVE_SCRATCH - 1] = sScratchDescription,
    [MOVE_VISE_GRIP - 1] = sViseGripDescription,
    [MOVE_GUILLOTINE - 1] = sGuillotineDescription,
    [MOVE_RAZOR_WIND - 1] = sRazorWindDescription,
    [MOVE_SWORDS_DANCE - 1] = sSwordsDanceDescription,
    [MOVE_CUT - 1] = sCutDescription,
    [MOVE_GUST - 1] = sGustDescription,
    [MOVE_WING_ATTACK - 1] = sWingAttackDescription,
    [MOVE_WHIRLWIND - 1] = sWhirlwindDescription,
    [MOVE_FLY - 1] = sFlyDescription,
    [MOVE_BIND - 1] = sBindDescription,
    [MOVE_SLAM - 1] = sSlamDescription,
    [MOVE_VINE_WHIP - 1] = sVineWhipDescription,
    [MOVE_STOMP - 1] = sStompDescription,
    [MOVE_DOUBLE_KICK - 1] = sDoubleKickDescription,
    [MOVE_MEGA_KICK - 1] = sMegaKickDescription,
    [MOVE_JUMP_KICK - 1] = sJumpKickDescription,
    [MOVE_ROLLING_KICK - 1] = sRollingKickDescription,
    [MOVE_SAND_ATTACK - 1] = sSandAttackDescription,
    [MOVE_HEADBUTT - 1] = sHeadbuttDescription,
    [MOVE_HORN_ATTACK - 1] = sHornAttackDescription,
    [MOVE_FURY_ATTACK - 1] = sFuryAttackDescription,
    [MOVE_HORN_DRILL - 1] = sHornDrillDescription,
    [MOVE_TACKLE - 1] = sTackleDescription,
    [MOVE_BODY_SLAM - 1] = sBodySlamDescription,
    [MOVE_WRAP - 1] = sWrapDescription,
    [MOVE_TAKE_DOWN - 1] = sTakeDownDescription,
    [MOVE_THRASH - 1] = sThrashDescription,
    [MOVE_DOUBLE_EDGE - 1] = sDoubleEdgeDescription,
    [MOVE_TAIL_WHIP - 1] = sTailWhipDescription,
    [MOVE_POISON_STING - 1] = sPoisonStingDescription,
    [MOVE_TWINEEDLE - 1] = sTwineedleDescription,
    [MOVE_PIN_MISSILE - 1] = sPinMissileDescription,
    [MOVE_LEER - 1] = sLeerDescription,
    [MOVE_BITE - 1] = sBiteDescription,
    [MOVE_GROWL - 1] = sGrowlDescription,
    [MOVE_ROAR - 1] = sRoarDescription,
    [MOVE_SING - 1] = sSingDescription,
    [MOVE_SUPERSONIC - 1] = sSupersonicDescription,
    [MOVE_SONIC_BOOM - 1] = sSonicBoomDescription,
    [MOVE_DISABLE - 1] = sDisableDescription,
    [MOVE_ACID - 1] = sAcidDescription,
    [MOVE_EMBER - 1] = sEmberDescription,
    [MOVE_FLAMETHROWER - 1] = sFlamethrowerDescription,
    [MOVE_MIST - 1] = sMistDescription,
    [MOVE_WATER_GUN - 1] = sWaterGunDescription,
    [MOVE_HYDRO_PUMP - 1] = sHydroPumpDescription,
    [MOVE_SURF - 1] = sSurfDescription,
    [MOVE_ICE_BEAM - 1] = sIceBeamDescription,
    [MOVE_BLIZZARD - 1] = sBlizzardDescription,
    [MOVE_PSYBEAM - 1] = sPsybeamDescription,
    [MOVE_BUBBLE_BEAM - 1] = sBubbleBeamDescription,
    [MOVE_AURORA_BEAM - 1] = sAuroraBeamDescription,
    [MOVE_HYPER_BEAM - 1] = sHyperBeamDescription,
    [MOVE_PECK - 1] = sPeckDescription,
    [MOVE_DRILL_PECK - 1] = sDrillPeckDescription,
    [MOVE_SUBMISSION - 1] = sSubmissionDescription,
    [MOVE_LOW_KICK - 1] = sLowKickDescription,
    [MOVE_COUNTER - 1] = sCounterDescription,
    [MOVE_SEISMIC_TOSS - 1] = sSeismicTossDescription,
    [MOVE_STRENGTH - 1] = sStrengthDescription,
    [MOVE_ABSORB - 1] = sAbsorbDescription,
    [MOVE_MEGA_DRAIN - 1] = sMegaDrainDescription,
    [MOVE_LEECH_SEED - 1] = sLeechSeedDescription,
    [MOVE_GROWTH - 1] = sGrowthDescription,
    [MOVE_RAZOR_LEAF - 1] = sRazorLeafDescription,
    [MOVE_SOLAR_BEAM - 1] = sSolarBeamDescription,
    [MOVE_POISON_POWDER - 1] = sPoisonPowderDescription,
    [MOVE_STUN_SPORE - 1] = sStunSporeDescription,
    [MOVE_SLEEP_POWDER - 1] = sSleepPowderDescription,
    [MOVE_PETAL_DANCE - 1] = sPetalDanceDescription,
    [MOVE_STRING_SHOT - 1] = sStringShotDescription,
    [MOVE_DRAGON_RAGE - 1] = sDragonRageDescription,
    [MOVE_FIRE_SPIN - 1] = sFireSpinDescription,
    [MOVE_THUNDER_SHOCK - 1] = sThunderShockDescription,
    [MOVE_THUNDERBOLT - 1] = sThunderboltDescription,
    [MOVE_THUNDER_WAVE - 1] = sThunderWaveDescription,
    [MOVE_THUNDER - 1] = sThunderDescription,
    [MOVE_ROCK_THROW - 1] = sRockThrowDescription,
    [MOVE_EARTHQUAKE - 1] = sEarthquakeDescription,
    [MOVE_FISSURE - 1] = sFissureDescription,
    [MOVE_DIG - 1] = sDigDescription,
    [MOVE_TOXIC - 1] = sToxicDescription,
    [MOVE_CONFUSION - 1] = sConfusionDescription,
    [MOVE_PSYCHIC - 1] = sPsychicDescription,
    [MOVE_HYPNOSIS - 1] = sHypnosisDescription,
    [MOVE_MEDITATE - 1] = sMeditateDescription,
    [MOVE_AGILITY - 1] = sAgilityDescription,
    [MOVE_QUICK_ATTACK - 1] = sQuickAttackDescription,
    [MOVE_RAGE - 1] = sRageDescription,
    [MOVE_TELEPORT - 1] = sTeleportDescription,
    [MOVE_NIGHT_SHADE - 1] = sNightShadeDescription,
    [MOVE_MIMIC - 1] = sMimicDescription,
    [MOVE_SCREECH - 1] = sScreechDescription,
    [MOVE_DOUBLE_TEAM - 1] = sDoubleTeamDescription,
    [MOVE_RECOVER - 1] = sRecoverDescription,
    [MOVE_HARDEN - 1] = sHardenDescription,
    [MOVE_MINIMIZE - 1] = sMinimizeDescription,
    [MOVE_SMOKESCREEN - 1] = sSmokescreenDescription,
    [MOVE_CONFUSE_RAY - 1] = sConfuseRayDescription,
    [MOVE_WITHDRAW - 1] = sWithdrawDescription,
    [MOVE_DEFENSE_CURL - 1] = sDefenseCurlDescription,
    [MOVE_BARRIER - 1] = sBarrierDescription,
    [MOVE_LIGHT_SCREEN - 1] = sLightScreenDescription,
    [MOVE_HAZE - 1] = sHazeDescription,
    [MOVE_REFLECT - 1] = sReflectDescription,
    [MOVE_FOCUS_ENERGY - 1] = sFocusEnergyDescription,
    [MOVE_BIDE - 1] = sBideDescription,
    [MOVE_METRONOME - 1] = sMetronomeDescription,
    [MOVE_MIRROR_MOVE - 1] = sMirrorMoveDescription,
    [MOVE_SELF_DESTRUCT - 1] = sSelfDestructDescription,
    [MOVE_EGG_BOMB - 1] = sEggBombDescription,
    [MOVE_LICK - 1] = sLickDescription,
    [MOVE_SMOG - 1] = sSmogDescription,
    [MOVE_SLUDGE - 1] = sSludgeDescription,
    [MOVE_BONE_CLUB - 1] = sBoneClubDescription,
    [MOVE_FIRE_BLAST - 1] = sFireBlastDescription,
    [MOVE_WATERFALL - 1] = sWaterfallDescription,
    [MOVE_CLAMP - 1] = sClampDescription,
    [MOVE_SWIFT - 1] = sSwiftDescription,
    [MOVE_SKULL_BASH - 1] = sSkullBashDescription,
    [MOVE_SPIKE_CANNON - 1] = sSpikeCannonDescription,
    [MOVE_CONSTRICT - 1] = sConstrictDescription,
    [MOVE_AMNESIA - 1] = sAmnesiaDescription,
    [MOVE_KINESIS - 1] = sKinesisDescription,
    [MOVE_SOFT_BOILED - 1] = sSoftBoiledDescription,
    [MOVE_HIGH_JUMP_KICK - 1] = sHighJumpKickDescription,
    [MOVE_GLARE - 1] = sGlareDescription,
    [MOVE_DREAM_EATER - 1] = sDreamEaterDescription,
    [MOVE_POISON_GAS - 1] = sPoisonGasDescription,
    [MOVE_BARRAGE - 1] = sBarrageDescription,
    [MOVE_LEECH_LIFE - 1] = sLeechLifeDescription,
    [MOVE_LOVELY_KISS - 1] = sLovelyKissDescription,
    [MOVE_SKY_ATTACK - 1] = sSkyAttackDescription,
    [MOVE_TRANSFORM - 1] = sTransformDescription,
    [MOVE_BUBBLE - 1] = sBubbleDescription,
    [MOVE_DIZZY_PUNCH - 1] = sDizzyPunchDescription,
    [MOVE_SPORE - 1] = sSporeDescription,
    [MOVE_FLASH - 1] = sFlashDescription,
    [MOVE_PSYWAVE - 1] = sPsywaveDescription,
    [MOVE_SPLASH - 1] = sSplashDescription,
    [MOVE_ACID_ARMOR - 1] = sAcidArmorDescription,
    [MOVE_CRABHAMMER - 1] = sCrabhammerDescription,
    [MOVE_EXPLOSION - 1] = sExplosionDescription,
    [MOVE_FURY_SWIPES - 1] = sFurySwipesDescription,
    [MOVE_BONEMERANG - 1] = sBonemerangDescription,
    [MOVE_REST - 1] = sRestDescription,
    [MOVE_ROCK_SLIDE - 1] = sRockSlideDescription,
    [MOVE_HYPER_FANG - 1] = sHyperFangDescription,
    [MOVE_SHARPEN - 1] = sSharpenDescription,
    [MOVE_CONVERSION - 1] = sConversionDescription,
    [MOVE_TRI_ATTACK - 1] = sTriAttackDescription,
    [MOVE_SUPER_FANG - 1] = sSuperFangDescription,
    [MOVE_SLASH - 1] = sSlashDescription,
    [MOVE_SUBSTITUTE - 1] = sSubstituteDescription,
    [MOVE_STRUGGLE - 1] = sStruggleDescription,
    [MOVE_SKETCH - 1] = sSketchDescription,
    [MOVE_TRIPLE_KICK - 1] = sTripleKickDescription,
    [MOVE_THIEF - 1] = sThiefDescription,
    [MOVE_SPIDER_WEB - 1] = sSpiderWebDescription,
    [MOVE_MIND_READER - 1] = sMindReaderDescription,
    [MOVE_NIGHTMARE - 1] = sNightmareDescription,
    [MOVE_FLAME_WHEEL - 1] = sFlameWheelDescription,
    [MOVE_SNORE - 1] = sSnoreDescription,
    [MOVE_CURSE - 1] = sCurseDescription,
    [MOVE_FLAIL - 1] = sFlailDescription,
    [MOVE_CONVERSION_2 - 1] = sConversion2Description,
    [MOVE_AEROBLAST - 1] = sAeroblastDescription,
    [MOVE_COTTON_SPORE - 1] = sCottonSporeDescription,
    [MOVE_REVERSAL - 1] = sReversalDescription,
    [MOVE_SPITE - 1] = sSpiteDescription,
    [MOVE_POWDER_SNOW - 1] = sPowderSnowDescription,
    [MOVE_PROTECT - 1] = sProtectDescription,
    [MOVE_MACH_PUNCH - 1] = sMachPunchDescription,
    [MOVE_SCARY_FACE - 1] = sScaryFaceDescription,
    [MOVE_FEINT_ATTACK - 1] = sFeintAttackDescription,
    [MOVE_SWEET_KISS - 1] = sSweetKissDescription,
    [MOVE_BELLY_DRUM - 1] = sBellyDrumDescription,
    [MOVE_SLUDGE_BOMB - 1] = sSludgeBombDescription,
    [MOVE_MUD_SLAP - 1] = sMudSlapDescription,
    [MOVE_OCTAZOOKA - 1] = sOctazookaDescription,
    [MOVE_SPIKES - 1] = sSpikesDescription,
    [MOVE_ZAP_CANNON - 1] = sZapCannonDescription,
    [MOVE_FORESIGHT - 1] = sForesightDescription,
    [MOVE_DESTINY_BOND - 1] = sDestinyBondDescription,
    [MOVE_PERISH_SONG - 1] = sPerishSongDescription,
    [MOVE_ICY_WIND - 1] = sIcyWindDescription,
    [MOVE_DETECT - 1] = sDetectDescription,
    [MOVE_BONE_RUSH - 1] = sBoneRushDescription,
    [MOVE_LOCK_ON - 1] = sLockOnDescription,
    [MOVE_OUTRAGE - 1] = sOutrageDescription,
    [MOVE_SANDSTORM - 1] = sSandstormDescription,
    [MOVE_GIGA_DRAIN - 1] = sGigaDrainDescription,
    [MOVE_ENDURE - 1] = sEndureDescription,
    [MOVE_CHARM - 1] = sCharmDescription,
    [MOVE_ROLLOUT - 1] = sRolloutDescription,
    [MOVE_FALSE_SWIPE - 1] = sFalseSwipeDescription,
    [MOVE_SWAGGER - 1] = sSwaggerDescription,
    [MOVE_MILK_DRINK - 1] = sMilkDrinkDescription,
    [MOVE_SPARK - 1] = sSparkDescription,
    [MOVE_FURY_CUTTER - 1] = sFuryCutterDescription,
    [MOVE_STEEL_WING - 1] = sSteelWingDescription,
    [MOVE_MEAN_LOOK - 1] = sMeanLookDescription,
    [MOVE_ATTRACT - 1] = sAttractDescription,
    [MOVE_SLEEP_TALK - 1] = sSleepTalkDescription,
    [MOVE_HEAL_BELL - 1] = sHealBellDescription,
    [MOVE_RETURN - 1] = sReturnDescription,
    [MOVE_PRESENT - 1] = sPresentDescription,
    [MOVE_FRUSTRATION - 1] = sFrustrationDescription,
    [MOVE_SAFEGUARD - 1] = sSafeguardDescription,
    [MOVE_PAIN_SPLIT - 1] = sPainSplitDescription,
    [MOVE_SACRED_FIRE - 1] = sSacredFireDescription,
    [MOVE_MAGNITUDE - 1] = sMagnitudeDescription,
    [MOVE_DYNAMIC_PUNCH - 1] = sDynamicPunchDescription,
    [MOVE_MEGAHORN - 1] = sMegahornDescription,
    [MOVE_DRAGON_BREATH - 1] = sDragonBreathDescription,
    [MOVE_BATON_PASS - 1] = sBatonPassDescription,
    [MOVE_ENCORE - 1] = sEncoreDescription,
    [MOVE_PURSUIT - 1] = sPursuitDescription,
    [MOVE_RAPID_SPIN - 1] = sRapidSpinDescription,
    [MOVE_SWEET_SCENT - 1] = sSweetScentDescription,
    [MOVE_IRON_TAIL - 1] = sIronTailDescription,
    [MOVE_METAL_CLAW - 1] = sMetalClawDescription,
    [MOVE_VITAL_THROW - 1] = sVitalThrowDescription,
    [MOVE_MORNING_SUN - 1] = sMorningSunDescription,
    [MOVE_SYNTHESIS - 1] = sSynthesisDescription,
    [MOVE_MOONLIGHT - 1] = sMoonlightDescription,
    [MOVE_HIDDEN_POWER - 1] = sHiddenPowerDescription,
    [MOVE_CROSS_CHOP - 1] = sCrossChopDescription,
    [MOVE_TWISTER - 1] = sTwisterDescription,
    [MOVE_RAIN_DANCE - 1] = sRainDanceDescription,
    [MOVE_SUNNY_DAY - 1] = sSunnyDayDescription,
    [MOVE_CRUNCH - 1] = sCrunchDescription,
    [MOVE_MIRROR_COAT - 1] = sMirrorCoatDescription,
    [MOVE_PSYCH_UP - 1] = sPsychUpDescription,
    [MOVE_EXTREME_SPEED - 1] = sExtremeSpeedDescription,
    [MOVE_ANCIENT_POWER - 1] = sAncientPowerDescription,
    [MOVE_SHADOW_BALL - 1] = sShadowBallDescription,
    [MOVE_FUTURE_SIGHT - 1] = sFutureSightDescription,
    [MOVE_ROCK_SMASH - 1] = sRockSmashDescription,
    [MOVE_WHIRLPOOL - 1] = sWhirlpoolDescription,
    [MOVE_BEAT_UP - 1] = sBeatUpDescription,
    [MOVE_FAKE_OUT - 1] = sFakeOutDescription,
    [MOVE_UPROAR - 1] = sUproarDescription,
    [MOVE_STOCKPILE - 1] = sStockpileDescription,
    [MOVE_SPIT_UP - 1] = sSpitUpDescription,
    [MOVE_SWALLOW - 1] = sSwallowDescription,
    [MOVE_HEAT_WAVE - 1] = sHeatWaveDescription,
    [MOVE_HAIL - 1] = sHailDescription,
    [MOVE_TORMENT - 1] = sTormentDescription,
    [MOVE_FLATTER - 1] = sFlatterDescription,
    [MOVE_WILL_O_WISP - 1] = sWillOWispDescription,
    [MOVE_MEMENTO - 1] = sMementoDescription,
    [MOVE_FACADE - 1] = sFacadeDescription,
    [MOVE_FOCUS_PUNCH - 1] = sFocusPunchDescription,
    [MOVE_SMELLING_SALTS - 1] = sSmellingSaltsDescription,
    [MOVE_FOLLOW_ME - 1] = sFollowMeDescription,
    [MOVE_NATURE_POWER - 1] = sNaturePowerDescription,
    [MOVE_CHARGE - 1] = sChargeDescription,
    [MOVE_TAUNT - 1] = sTauntDescription,
    [MOVE_HELPING_HAND - 1] = sHelpingHandDescription,
    [MOVE_TRICK - 1] = sTrickDescription,
    [MOVE_ROLE_PLAY - 1] = sRolePlayDescription,
    [MOVE_WISH - 1] = sWishDescription,
    [MOVE_ASSIST - 1] = sAssistDescription,
    [MOVE_INGRAIN - 1] = sIngrainDescription,
    [MOVE_SUPERPOWER - 1] = sSuperpowerDescription,
    [MOVE_MAGIC_COAT - 1] = sMagicCoatDescription,
    [MOVE_RECYCLE - 1] = sRecycleDescription,
    [MOVE_REVENGE - 1] = sRevengeDescription,
    [MOVE_BRICK_BREAK - 1] = sBrickBreakDescription,
    [MOVE_YAWN - 1] = sYawnDescription,
    [MOVE_KNOCK_OFF - 1] = sKnockOffDescription,
    [MOVE_ENDEAVOR - 1] = sEndeavorDescription,
    [MOVE_ERUPTION - 1] = sEruptionDescription,
    [MOVE_SKILL_SWAP - 1] = sSkillSwapDescription,
    [MOVE_IMPRISON - 1] = sImprisonDescription,
    [MOVE_REFRESH - 1] = sRefreshDescription,
    [MOVE_GRUDGE - 1] = sGrudgeDescription,
    [MOVE_SNATCH - 1] = sSnatchDescription,
    [MOVE_SECRET_POWER - 1] = sSecretPowerDescription,
    [MOVE_DIVE - 1] = sDiveDescription,
    [MOVE_ARM_THRUST - 1] = sArmThrustDescription,
    [MOVE_CAMOUFLAGE - 1] = sCamouflageDescription,
    [MOVE_TAIL_GLOW - 1] = sTailGlowDescription,
    [MOVE_LUSTER_PURGE - 1] = sLusterPurgeDescription,
    [MOVE_MIST_BALL - 1] = sMistBallDescription,
    [MOVE_FEATHER_DANCE - 1] = sFeatherDanceDescription,
    [MOVE_TEETER_DANCE - 1] = sTeeterDanceDescription,
    [MOVE_BLAZE_KICK - 1] = sBlazeKickDescription,
    [MOVE_MUD_SPORT - 1] = sMudSportDescription,
    [MOVE_ICE_BALL - 1] = sIceBallDescription,
    [MOVE_NEEDLE_ARM - 1] = sNeedleArmDescription,
    [MOVE_SLACK_OFF - 1] = sSlackOffDescription,
    [MOVE_HYPER_VOICE - 1] = sHyperVoiceDescription,
    [MOVE_POISON_FANG - 1] = sPoisonFangDescription,
    [MOVE_CRUSH_CLAW - 1] = sCrushClawDescription,
    [MOVE_BLAST_BURN - 1] = sBlastBurnDescription,
    [MOVE_HYDRO_CANNON - 1] = sHydroCannonDescription,
    [MOVE_METEOR_MASH - 1] = sMeteorMashDescription,
    [MOVE_ASTONISH - 1] = sAstonishDescription,
    [MOVE_WEATHER_BALL - 1] = sWeatherBallDescription,
    [MOVE_AROMATHERAPY - 1] = sAromatherapyDescription,
    [MOVE_FAKE_TEARS - 1] = sFakeTearsDescription,
    [MOVE_AIR_CUTTER - 1] = sAirCutterDescription,
    [MOVE_OVERHEAT - 1] = sOverheatDescription,
    [MOVE_ODOR_SLEUTH - 1] = sOdorSleuthDescription,
    [MOVE_ROCK_TOMB - 1] = sRockTombDescription,
    [MOVE_SILVER_WIND - 1] = sSilverWindDescription,
    [MOVE_METAL_SOUND - 1] = sMetalSoundDescription,
    [MOVE_GRASS_WHISTLE - 1] = sGrassWhistleDescription,
    [MOVE_TICKLE - 1] = sTickleDescription,
    [MOVE_COSMIC_POWER - 1] = sCosmicPowerDescription,
    [MOVE_WATER_SPOUT - 1] = sWaterSpoutDescription,
    [MOVE_SIGNAL_BEAM - 1] = sSignalBeamDescription,
    [MOVE_SHADOW_PUNCH - 1] = sShadowPunchDescription,
    [MOVE_EXTRASENSORY - 1] = sExtrasensoryDescription,
    [MOVE_SKY_UPPERCUT - 1] = sSkyUppercutDescription,
    [MOVE_SAND_TOMB - 1] = sSandTombDescription,
    [MOVE_SHEER_COLD - 1] = sSheerColdDescription,
    [MOVE_MUDDY_WATER - 1] = sMuddyWaterDescription,
    [MOVE_BULLET_SEED - 1] = sBulletSeedDescription,
    [MOVE_AERIAL_ACE - 1] = sAerialAceDescription,
    [MOVE_ICICLE_SPEAR - 1] = sIcicleSpearDescription,
    [MOVE_IRON_DEFENSE - 1] = sIronDefenseDescription,
    [MOVE_BLOCK - 1] = sBlockDescription,
    [MOVE_HOWL - 1] = sHowlDescription,
    [MOVE_DRAGON_CLAW - 1] = sDragonClawDescription,
    [MOVE_FRENZY_PLANT - 1] = sFrenzyPlantDescription,
    [MOVE_BULK_UP - 1] = sBulkUpDescription,
    [MOVE_BOUNCE - 1] = sBounceDescription,
    [MOVE_MUD_SHOT - 1] = sMudShotDescription,
    [MOVE_POISON_TAIL - 1] = sPoisonTailDescription,
    [MOVE_COVET - 1] = sCovetDescription,
    [MOVE_VOLT_TACKLE - 1] = sVoltTackleDescription,
    [MOVE_MAGICAL_LEAF - 1] = sMagicalLeafDescription,
    [MOVE_WATER_SPORT - 1] = sWaterSportDescription,
    [MOVE_CALM_MIND - 1] = sCalmMindDescription,
    [MOVE_LEAF_BLADE - 1] = sLeafBladeDescription,
    [MOVE_DRAGON_DANCE - 1] = sDragonDanceDescription,
    [MOVE_ROCK_BLAST - 1] = sRockBlastDescription,
    [MOVE_SHOCK_WAVE - 1] = sShockWaveDescription,
    [MOVE_WATER_PULSE - 1] = sWaterPulseDescription,
    [MOVE_DOOM_DESIRE - 1] = sDoomDesireDescription,
    [MOVE_PSYCHO_BOOST - 1] = sPsychoBoostDescription,
    [MOVE_ROOST - 1] = sRoostDescription,
    [MOVE_GRAVITY - 1] = sGravityDescription,
    [MOVE_MIRACLE_EYE - 1] = sMiracleEyeDescription,
    [MOVE_WAKE_UP_SLAP - 1] = sWakeUpSlapDescription,
    [MOVE_HAMMER_ARM - 1] = sHammerArmDescription,
    [MOVE_GYRO_BALL - 1] = sGyroBallDescription,
    [MOVE_HEALING_WISH - 1] = sHealingWishDescription,
    [MOVE_BRINE - 1] = sBrineDescription,
    [MOVE_NATURAL_GIFT - 1] = sNaturalGiftDescription,
    [MOVE_FEINT - 1] = sFeintDescription,
    [MOVE_PLUCK - 1] = sPluckDescription,
    [MOVE_TAILWIND - 1] = sTailwindDescription,
    [MOVE_ACUPRESSURE - 1] = sAcupressureDescription,
    [MOVE_METAL_BURST - 1] = sMetalBurstDescription,
    [MOVE_U_TURN - 1] = sUTurnDescription,
    [MOVE_CLOSE_COMBAT - 1] = sCloseCombatDescription,
    [MOVE_PAYBACK - 1] = sPaybackDescription,
    [MOVE_ASSURANCE - 1] = sAssuranceDescription,
    [MOVE_EMBARGO - 1] = sEmbargoDescription,
    [MOVE_FLING - 1] = sFlingDescription,
    [MOVE_PSYCHO_SHIFT - 1] = sPsychoShiftDescription,
    [MOVE_TRUMP_CARD - 1] = sTrumpCardDescription,
    [MOVE_HEAL_BLOCK - 1] = sHealBlockDescription,
    [MOVE_WRING_OUT - 1] = sWringOutDescription,
    [MOVE_POWER_TRICK - 1] = sPowerTrickDescription,
    [MOVE_GASTRO_ACID - 1] = sGastroAcidDescription,
    [MOVE_LUCKY_CHANT - 1] = sLuckyChantDescription,
    [MOVE_ME_FIRST - 1] = sMeFirstDescription,
    [MOVE_COPYCAT - 1] = sCopycatDescription,
    [MOVE_POWER_SWAP - 1] = sPowerSwapDescription,
    [MOVE_GUARD_SWAP - 1] = sGuardSwapDescription,
    [MOVE_PUNISHMENT - 1] = sPunishmentDescription,
    [MOVE_LAST_RESORT - 1] = sLastResortDescription,
    [MOVE_WORRY_SEED - 1] = sWorrySeedDescription,
    [MOVE_SUCKER_PUNCH - 1] = sSuckerPunchDescription,
    [MOVE_TOXIC_SPIKES - 1] = sToxicSpikesDescription,
    [MOVE_HEART_SWAP - 1] = sHeartSwapDescription,
    [MOVE_AQUA_RING - 1] = sAquaRingDescription,
    [MOVE_MAGNET_RISE - 1] = sMagnetRiseDescription,
    [MOVE_FLARE_BLITZ - 1] = sFlareBlitzDescription,
    [MOVE_FORCE_PALM - 1] = sForcePalmDescription,
    [MOVE_AURA_SPHERE - 1] = sAuraSphereDescription,
    [MOVE_ROCK_POLISH - 1] = sRockPolishDescription,
    [MOVE_POISON_JAB - 1] = sPoisonJabDescription,
    [MOVE_DARK_PULSE - 1] = sDarkPulseDescription,
    [MOVE_NIGHT_SLASH - 1] = sNightSlashDescription,
    [MOVE_AQUA_TAIL - 1] = sAquaTailDescription,
    [MOVE_SEED_BOMB - 1] = sSeedBombDescription,
    [MOVE_AIR_SLASH - 1] = sAirSlashDescription,
    [MOVE_X_SCISSOR - 1] = sXScissorDescription,
    [MOVE_BUG_BUZZ - 1] = sBugBuzzDescription,
    [MOVE_DRAGON_PULSE - 1] = sDragonPulseDescription,
    [MOVE_DRAGON_RUSH - 1] = sDragonRushDescription,
    [MOVE_POWER_GEM - 1] = sPowerGemDescription,
    [MOVE_DRAIN_PUNCH - 1] = sMegaDrainDescription,
    [MOVE_VACUUM_WAVE - 1] = sVacuumWaveDescription,
    [MOVE_FOCUS_BLAST - 1] = sFocusBlastDescription,
    [MOVE_ENERGY_BALL - 1] = sEnergyBallDescription,
    [MOVE_BRAVE_BIRD - 1] = sBraveBirdDescription,
    [MOVE_EARTH_POWER - 1] = sEarthPowerDescription,
    [MOVE_SWITCHEROO - 1] = sSwitcherooDescription,
    [MOVE_GIGA_IMPACT - 1] = sHyperBeamDescription,
    [MOVE_NASTY_PLOT - 1] = sNastyPlotDescription,
    [MOVE_BULLET_PUNCH - 1] = sBulletPunchDescription,
    [MOVE_AVALANCHE - 1] = sRevengeDescription,
    [MOVE_ICE_SHARD - 1] = sIceShardDescription,
    [MOVE_SHADOW_CLAW - 1] = sShadowClawDescription,
    [MOVE_THUNDER_FANG - 1] = sThunderFangDescription,
    [MOVE_ICE_FANG - 1] = sIceFangDescription,
    [MOVE_FIRE_FANG - 1] = sFireFangDescription,
    [MOVE_SHADOW_SNEAK - 1] = sShadowSneakDescription,
    [MOVE_MUD_BOMB - 1] = sMudBombDescription,
    [MOVE_PSYCHO_CUT - 1] = sPsychoCutDescription,
    [MOVE_ZEN_HEADBUTT - 1] = sZenHeadbuttDescription,
    [MOVE_MIRROR_SHOT - 1] = sMirrorShotDescription,
    [MOVE_FLASH_CANNON - 1] = sFlashCannonDescription,
    [MOVE_ROCK_CLIMB - 1] = sRockClimbDescription,
    [MOVE_DEFOG - 1] = sDefogDescription,
    [MOVE_TRICK_ROOM - 1] = sTrickRoomDescription,
    [MOVE_DRACO_METEOR - 1] = sDracoMeteorDescription,
    [MOVE_DISCHARGE - 1] = sDischargeDescription,
    [MOVE_LAVA_PLUME - 1] = sLavaPlumeDescription,
    [MOVE_LEAF_STORM - 1] = sLeafStormDescription,
    [MOVE_POWER_WHIP - 1] = sPowerWhipDescription,
    [MOVE_ROCK_WRECKER - 1] = sHyperBeamDescription,
    [MOVE_CROSS_POISON - 1] = sCrossPoisonDescription,
    [MOVE_GUNK_SHOT - 1] = sGunkShotDescription,
    [MOVE_IRON_HEAD - 1] = sIronHeadDescription,
    [MOVE_MAGNET_BOMB - 1] = sMagnetBombDescription,
    [MOVE_STONE_EDGE - 1] = sStoneEdgeDescription,
    [MOVE_CAPTIVATE - 1] = sCaptivateDescription,
    [MOVE_STEALTH_ROCK - 1] = sStealthRockDescription,
    [MOVE_GRASS_KNOT - 1] = sGrassKnotDescription,
    [MOVE_CHATTER - 1] = sChatterDescription,
    [MOVE_JUDGMENT - 1] = sJudgmentDescription,
    [MOVE_BUG_BITE - 1] = sPluckDescription,
    [MOVE_CHARGE_BEAM - 1] = sChargeBeamDescription,
    [MOVE_WOOD_HAMMER - 1] = sWoodHammerDescription,
    [MOVE_AQUA_JET - 1] = sAquaJetDescription,
    [MOVE_ATTACK_ORDER - 1] = sAttackOrderDescription,
    [MOVE_DEFEND_ORDER - 1] = sDefendOrderDescription,
    [MOVE_HEAL_ORDER - 1] = sHealOrderDescription,
    [MOVE_HEAD_SMASH - 1] = sHeadSmashDescription,
    [MOVE_DOUBLE_HIT - 1] = sDoubleHitDescription,
    [MOVE_ROAR_OF_TIME - 1] = sRoarOfTimeDescription,
    [MOVE_SPACIAL_REND - 1] = sSpacialRendDescription,
    [MOVE_LUNAR_DANCE - 1] = sHealingWishDescription,
    [MOVE_CRUSH_GRIP - 1] = sWringOutDescription,
    [MOVE_MAGMA_STORM - 1] = sMagmaStormDescription,
    [MOVE_DARK_VOID - 1] = sDarkVoidDescription,
    [MOVE_SEED_FLARE - 1] = sSeedFlareDescription,
    [MOVE_OMINOUS_WIND - 1] = sOminousWindDescription,
    [MOVE_SHADOW_FORCE - 1] = sShadowForceDescription,
    [MOVE_HONE_CLAWS - 1] = sHoneClawsDescription,
    [MOVE_WIDE_GUARD - 1] = sWideGuardDescription,
    [MOVE_GUARD_SPLIT - 1] = sGuardSplitDescription,
    [MOVE_POWER_SPLIT - 1] = sPowerSplitDescription,
    [MOVE_WONDER_ROOM - 1] = sWonderRoomDescription,
    [MOVE_PSYSHOCK - 1] = sPsyshockDescription,
    [MOVE_VENOSHOCK - 1] = sVenoshockDescription,
    [MOVE_AUTOTOMIZE - 1] = sAutotomizeDescription,
    [MOVE_RAGE_POWDER - 1] = sRagePowderDescription,
    [MOVE_TELEKINESIS - 1] = sTelekinesisDescription,
    [MOVE_MAGIC_ROOM - 1] = sMagicRoomDescription,
    [MOVE_SMACK_DOWN - 1] = sSmackDownDescription,
    [MOVE_STORM_THROW - 1] = sStormThrowDescription,
    [MOVE_FLAME_BURST - 1] = sFlameBurstDescription,
    [MOVE_SLUDGE_WAVE - 1] = sSludgeWaveDescription,
    [MOVE_QUIVER_DANCE - 1] = sQuiverDanceDescription,
    [MOVE_HEAVY_SLAM - 1] = sHeavySlamDescription,
    [MOVE_SYNCHRONOISE - 1] = sSynchronoiseDescription,
    [MOVE_ELECTRO_BALL - 1] = sElectroBallDescription,
    [MOVE_SOAK - 1] = sSoakDescription,
    [MOVE_FLAME_CHARGE - 1] = sFlameChargeDescription,
    [MOVE_COIL - 1] = sCoilDescription,
    [MOVE_LOW_SWEEP - 1] = sLowSweepDescription,
    [MOVE_ACID_SPRAY - 1] = sAcidSprayDescription,
    [MOVE_FOUL_PLAY - 1] = sFoulPlayDescription,
    [MOVE_SIMPLE_BEAM - 1] = sSimpleBeamDescription,
    [MOVE_ENTRAINMENT - 1] = sEntrainmentDescription,
    [MOVE_AFTER_YOU - 1] = sAfterYouDescription,
    [MOVE_ROUND - 1] = sRoundDescription,
    [MOVE_ECHOED_VOICE - 1] = sEchoedVoiceDescription,
    [MOVE_CHIP_AWAY - 1] = sChipAwayDescription,
    [MOVE_CLEAR_SMOG - 1] = sClearSmogDescription,
    [MOVE_STORED_POWER - 1] = sStoredPowerDescription,
    [MOVE_QUICK_GUARD - 1] = sQuickGuardDescription,
    [MOVE_ALLY_SWITCH - 1] = sAllySwitchDescription,
    [MOVE_SCALD - 1] = sScaldDescription,
    [MOVE_SHELL_SMASH - 1] = sShellSmashDescription,
    [MOVE_HEAL_PULSE - 1] = sHealPulseDescription,
    [MOVE_HEX - 1] = sHexDescription,
    [MOVE_SKY_DROP - 1] = sSkyDropDescription,
    [MOVE_SHIFT_GEAR - 1] = sShiftGearDescription,
    [MOVE_CIRCLE_THROW - 1] = sCircleThrowDescription,
    [MOVE_INCINERATE - 1] = sIncinerateDescription,
    [MOVE_QUASH - 1] = sQuashDescription,
    [MOVE_ACROBATICS - 1] = sAcrobaticsDescription,
    [MOVE_REFLECT_TYPE - 1] = sReflectTypeDescription,
    [MOVE_RETALIATE - 1] = sRetaliateDescription,
    [MOVE_FINAL_GAMBIT - 1] = sFinalGambitDescription,
    [MOVE_BESTOW - 1] = sBestowDescription,
    [MOVE_INFERNO - 1] = sInfernoDescription,
    [MOVE_WATER_PLEDGE - 1] = sWaterPledgeDescription,
    [MOVE_FIRE_PLEDGE - 1] = sFirePledgeDescription,
    [MOVE_GRASS_PLEDGE - 1] = sGrassPledgeDescription,
    [MOVE_VOLT_SWITCH - 1] = sUTurnDescription,
    [MOVE_STRUGGLE_BUG - 1] = sStruggleBugDescription,
    [MOVE_BULLDOZE - 1] = sBulldozeDescription,
    [MOVE_FROST_BREATH - 1] = sStormThrowDescription,
    [MOVE_DRAGON_TAIL - 1] = sCircleThrowDescription,
    [MOVE_WORK_UP - 1] = sWorkUpDescription,
    [MOVE_ELECTROWEB - 1] = sElectrowebDescription,
    [MOVE_WILD_CHARGE - 1] = sWildChargeDescription,
    [MOVE_DRILL_RUN - 1] = sDrillRunDescription,
    [MOVE_DUAL_CHOP - 1] = sDualChopDescription,
    [MOVE_HEART_STAMP - 1] = sHeartStampDescription,
    [MOVE_HORN_LEECH - 1] = sMegaDrainDescription,
    [MOVE_SACRED_SWORD - 1] = sChipAwayDescription,
    [MOVE_RAZOR_SHELL - 1] = sRazorShellDescription,
    [MOVE_HEAT_CRASH - 1] = sHeavySlamDescription,
    [MOVE_LEAF_TORNADO - 1] = sLeafTornadoDescription,
    [MOVE_STEAMROLLER - 1] = sSteamrollerDescription,
    [MOVE_COTTON_GUARD - 1] = sCottonGuardDescription,
    [MOVE_NIGHT_DAZE - 1] = sNightDazeDescription,
    [MOVE_PSYSTRIKE - 1] = sPsyshockDescription,
    [MOVE_TAIL_SLAP - 1] = sTailSlapDescription,
    [MOVE_HURRICANE - 1] = sHurricaneDescription,
    [MOVE_HEAD_CHARGE - 1] = sHeadChargeDescription,
    [MOVE_GEAR_GRIND - 1] = sGearGrindDescription,
    [MOVE_SEARING_SHOT - 1] = sLavaPlumeDescription,
    [MOVE_TECHNO_BLAST - 1] = sTechnoBlastDescription,
    [MOVE_RELIC_SONG - 1] = sRelicSongDescription,
    [MOVE_SECRET_SWORD - 1] = sSecretSwordDescription,
    [MOVE_GLACIATE - 1] = sGlaciateDescription,
    [MOVE_BOLT_STRIKE - 1] = sBoltStrikeDescription,
    [MOVE_BLUE_FLARE - 1] = sBlueFlareDescription,
    [MOVE_FIERY_DANCE - 1] = sFieryDanceDescription,
    [MOVE_FREEZE_SHOCK - 1] = sFreezeShockDescription,
    [MOVE_ICE_BURN - 1] = sIceBurnDescription,
    [MOVE_SNARL - 1] = sSnarlDescription,
    [MOVE_ICICLE_CRASH - 1] = sIcicleCrashDescription,
    [MOVE_V_CREATE - 1] = sVCreateDescription,
    [MOVE_FUSION_FLARE - 1] = sFusionFlareDescription,
    [MOVE_FUSION_BOLT - 1] = sFusionBoltDescription,
    [MOVE_FLYING_PRESS - 1] = sFlyingPressDescription,
    [MOVE_MAT_BLOCK - 1] = sMatBlockDescription,
    [MOVE_BELCH - 1] = sBelchDescription,
    [MOVE_ROTOTILLER - 1] = sRototillerDescription,
    [MOVE_STICKY_WEB - 1] = sStickyWebDescription,
    [MOVE_FELL_STINGER - 1] = sFellStingerDescription,
    [MOVE_PHANTOM_FORCE - 1] = sShadowForceDescription,
    [MOVE_TRICK_OR_TREAT - 1] = sTrickOrTreatDescription,
    [MOVE_NOBLE_ROAR - 1] = sNobleRoarDescription,
    [MOVE_ION_DELUGE - 1] = sIonDelugeDescription,
    [MOVE_PARABOLIC_CHARGE - 1] = sParabolicChargeDescription,
    [MOVE_FORESTS_CURSE - 1] = sForestsCurseDescription,
    [MOVE_PETAL_BLIZZARD - 1] = sPetalBlizzardDescription,
    [MOVE_FREEZE_DRY - 1] = sFreezeDryDescription,
    [MOVE_DISARMING_VOICE - 1] = sDisarmingVoiceDescription,
    [MOVE_PARTING_SHOT - 1] = sPartingShotDescription,
    [MOVE_TOPSY_TURVY - 1] = sTopsyTurvyDescription,
    [MOVE_DRAINING_KISS - 1] = sDrainingKissDescription,
    [MOVE_CRAFTY_SHIELD - 1] = sCraftyShieldDescription,
    [MOVE_FLOWER_SHIELD - 1] = sFlowerShieldDescription,
    [MOVE_GRASSY_TERRAIN - 1] = sGrassyTerrainDescription,
    [MOVE_MISTY_TERRAIN - 1] = sMistyTerrainDescription,
    [MOVE_ELECTRIFY - 1] = sElectrifyDescription,
    [MOVE_PLAY_ROUGH - 1] = sPlayRoughDescription,
    [MOVE_FAIRY_WIND - 1] = sFairyWindDescription,
    [MOVE_MOONBLAST - 1] = sMoonblastDescription,
    [MOVE_BOOMBURST - 1] = sBoomburstDescription,
    [MOVE_FAIRY_LOCK - 1] = sFairyLockDescription,
    [MOVE_KINGS_SHIELD - 1] = sKingsShieldDescription,
    [MOVE_PLAY_NICE - 1] = sPlayNiceDescription,
    [MOVE_CONFIDE - 1] = sConfideDescription,
    [MOVE_DIAMOND_STORM - 1] = sDiamondStormDescription,
    [MOVE_STEAM_ERUPTION - 1] = sSteamEruptionDescription,
    [MOVE_HYPERSPACE_HOLE - 1] = sHyperspaceHoleDescription,
    [MOVE_WATER_SHURIKEN - 1] = sWaterShurikenDescription,
    [MOVE_MYSTICAL_FIRE - 1] = sMysticalFireDescription,
    [MOVE_SPIKY_SHIELD - 1] = sSpikyShieldDescription,
    [MOVE_AROMATIC_MIST - 1] = sAromaticMistDescription,
    [MOVE_EERIE_IMPULSE - 1] = sEerieImpulseDescription,
    [MOVE_VENOM_DRENCH - 1] = sVenomDrenchDescription,
    [MOVE_POWDER - 1] = sPowderDescription,
    [MOVE_GEOMANCY - 1] = sGeomancyDescription,
    [MOVE_MAGNETIC_FLUX - 1] = sMagneticFluxDescription,
    [MOVE_HAPPY_HOUR - 1] = sHappyHourDescription,
    [MOVE_ELECTRIC_TERRAIN - 1] = sElectricTerrainDescription,
    [MOVE_DAZZLING_GLEAM - 1] = sDazzlingGleamDescription,
    [MOVE_CELEBRATE - 1] = sCelebrateDescription,
    [MOVE_HOLD_HANDS - 1] = sHoldHandsDescription,
    [MOVE_BABY_DOLL_EYES - 1] = sBabyDollEyesDescription,
    [MOVE_NUZZLE - 1] = sNuzzleDescription,
    [MOVE_HOLD_BACK - 1] = sFalseSwipeDescription,
    [MOVE_INFESTATION - 1] = sInfestationDescription,
    [MOVE_POWER_UP_PUNCH - 1] = sPowerUpPunchDescription,
    [MOVE_OBLIVION_WING - 1] = sDrainingKissDescription,
    [MOVE_THOUSAND_ARROWS - 1] = sThousandArrowsDescription,
    [MOVE_THOUSAND_WAVES - 1] = sThousandWavesDescription,
    [MOVE_LANDS_WRATH - 1] = sLandsWrathDescription,
    [MOVE_LIGHT_OF_RUIN - 1] = sLightOfRuinDescription,
    [MOVE_ORIGIN_PULSE - 1] = sOriginPulseDescription,
    [MOVE_PRECIPICE_BLADES - 1] = sPrecipiceBladesDescription,
    [MOVE_DRAGON_ASCENT - 1] = sCloseCombatDescription,
    [MOVE_HYPERSPACE_FURY - 1] = sHyperspaceHoleDescription,
    [MOVE_SHORE_UP - 1] = sShoreUpDescription,
    [MOVE_FIRST_IMPRESSION - 1] = sFirstImpressionDescription,
    [MOVE_BANEFUL_BUNKER - 1] = sBanefulBunkerDescription,
    [MOVE_SPIRIT_SHACKLE - 1] = sSpiritShackleDescription,
    [MOVE_DARKEST_LARIAT - 1] = sDarkestLariatDescription,
    [MOVE_SPARKLING_ARIA - 1] = sSparklingAriaDescription,
    [MOVE_ICE_HAMMER - 1] = sIceHammerDescription,
    [MOVE_FLORAL_HEALING - 1] = sFloralHealingDescription,
    [MOVE_HIGH_HORSEPOWER - 1] = sHighHorsepowerDescription,
    [MOVE_STRENGTH_SAP - 1] = sStrengthSapDescription,
    [MOVE_SOLAR_BLADE - 1] = sSolarBladeDescription,
    [MOVE_LEAFAGE - 1] = sLeafageDescription,
    [MOVE_SPOTLIGHT - 1] = sSpotlightDescription,
    [MOVE_TOXIC_THREAD - 1] = sToxicThreadDescription,
    [MOVE_LASER_FOCUS - 1] = sLaserFocusDescription,
    [MOVE_GEAR_UP - 1] = sGearUpDescription,
    [MOVE_THROAT_CHOP - 1] = sThroatChopDescription,
    [MOVE_POLLEN_PUFF - 1] = sPollenPuffDescription,
    [MOVE_ANCHOR_SHOT - 1] = sAnchorShotDescription,
    [MOVE_PSYCHIC_TERRAIN - 1] = sPsychicTerrainDescription,
    [MOVE_LUNGE - 1] = sLungeDescription,
    [MOVE_FIRE_LASH - 1] = sFireLashDescription,
    [MOVE_POWER_TRIP - 1] = sPowerTripDescription,
    [MOVE_BURN_UP - 1] = sBurnUpDescription,
    [MOVE_SPEED_SWAP - 1] = sSpeedSwapDescription,
    [MOVE_SMART_STRIKE - 1] = sSmartStrikeDescription,
    [MOVE_PURIFY - 1] = sPurifyDescription,
    [MOVE_REVELATION_DANCE - 1] = sRevelationDanceDescription,
    [MOVE_CORE_ENFORCER - 1] = sCoreEnforcerDescription,
    [MOVE_TROP_KICK - 1] = sTropKickDescription,
    [MOVE_INSTRUCT - 1] = sInstructDescription,
    [MOVE_BEAK_BLAST - 1] = sBeakBlastDescription,
    [MOVE_CLANGING_SCALES - 1] = sClangingScalesDescription,
    [MOVE_DRAGON_HAMMER - 1] = sDragonHammerDescription,
    [MOVE_BRUTAL_SWING - 1] = sBrutalSwingDescription,
    [MOVE_AURORA_VEIL - 1] = sAuroraVeilDescription,
    [MOVE_SHELL_TRAP - 1] = sShellTrapDescription,
    [MOVE_FLEUR_CANNON - 1] = sFleurCannonDescription,
    [MOVE_PSYCHIC_FANGS - 1] = sPsychicFangsDescription,
    [MOVE_STOMPING_TANTRUM - 1] = sStompingTantrumDescription,
    [MOVE_SHADOW_BONE - 1] = sShadowBoneDescription,
    [MOVE_ACCELEROCK - 1] = sAccelerockDescription,
    [MOVE_LIQUIDATION - 1] = sLiquidationDescription,
    [MOVE_PRISMATIC_LASER - 1] = sPrismaticLaserDescription,
    [MOVE_SPECTRAL_THIEF - 1] = sSpectralThiefDescription,
    [MOVE_SUNSTEEL_STRIKE - 1] = sSunsteelStrikeDescription,
    [MOVE_MOONGEIST_BEAM - 1] = sMoongeistBeamDescription,
    [MOVE_TEARFUL_LOOK - 1] = sTearfulLookDescription,
    [MOVE_ZING_ZAP - 1] = sZingZapDescription,
    [MOVE_NATURES_MADNESS - 1] = sNaturesMadnessDescription,
    [MOVE_MULTI_ATTACK - 1] = sMultiAttackDescription,
    [MOVE_MIND_BLOWN - 1] = sMindBlownDescription,
    [MOVE_PLASMA_FISTS - 1] = sPlasmaFistsDescription,
    [MOVE_PHOTON_GEYSER - 1] = sPhotonGeyserDescription,
    [MOVE_ZIPPY_ZAP - 1] = sZippyZapDescription,
    [MOVE_SPLISHY_SPLASH - 1] = sSplishySplashDescription,
    [MOVE_FLOATY_FALL - 1] = sFloatyFallDescription,
    [MOVE_PIKA_PAPOW - 1] = sPikaPapowDescription,
    [MOVE_BOUNCY_BUBBLE - 1] = sBouncyBubbleDescription,
    [MOVE_BUZZY_BUZZ - 1] = sBuzzyBuzzDescription,
    [MOVE_SIZZLY_SLIDE - 1] = sSizzlySlideDescription,
    [MOVE_GLITZY_GLOW - 1] = sGlitzyGlowDescription,
    [MOVE_BADDY_BAD - 1] = sBaddyBadDescription,
    [MOVE_SAPPY_SEED - 1] = sSappySeedDescription,
    [MOVE_FREEZY_FROST - 1] = sFreezyFrostDescription,
    [MOVE_SPARKLY_SWIRL - 1] = sSparklySwirlDescription,
    [MOVE_VEEVEE_VOLLEY - 1] = sVeeveeVolleyDescription,
    [MOVE_DOUBLE_IRON_BASH - 1] = sDoubleIronBashDescription,

    //GEN 8
    [MOVE_DYNAMAX_CANNON - 1] = sDynamaxCannonDescription,
    [MOVE_SNIPE_SHOT - 1] = sSnipeShotDescription,
    [MOVE_JAW_LOCK - 1] = sJawLockDescription,
    [MOVE_STUFF_CHEEKS - 1] = sStuffCheeksDescription,
    [MOVE_NO_RETREAT - 1] = sNoRetreatDescription,
    [MOVE_TAR_SHOT - 1] = sTarShotDescription,
    [MOVE_MAGIC_POWDER - 1] = sMagicPowderDescription,
    [MOVE_DRAGON_DARTS - 1] = sDragonDartsDescription,
    [MOVE_TEATIME - 1] = sTeatimeDescription,
    [MOVE_OCTOLOCK - 1] = sOctolockDescription,
    [MOVE_BOLT_BEAK - 1] = sBoltBeakDescription,
    [MOVE_FISHIOUS_REND - 1] = sFishiousRendDescription,
    [MOVE_COURT_CHANGE - 1] = sCourtChangeDescription,
    [MOVE_CLANGOROUS_SOUL - 1] = sClangorousSoulDescription,
    [MOVE_BODY_PRESS - 1] = sBodyPressDescription,
    [MOVE_DECORATE - 1] = sDecorateDescription,
    [MOVE_DRUM_BEATING - 1] = sDrumBeatingDescription,
    [MOVE_SNAP_TRAP - 1] = sSnapTrapDescription,
    [MOVE_PYRO_BALL - 1] = sPyroBallDescription,
    [MOVE_BEHEMOTH_BLADE - 1] = sBehemothBladeDescription,
    [MOVE_BEHEMOTH_BASH - 1] = sBehemothBashDescription,
    [MOVE_AURA_WHEEL - 1] = sAuraWheelDescription,
    [MOVE_BREAKING_SWIPE - 1] = sBreakingSwipeDescription,
    [MOVE_BRANCH_POKE - 1] = sBranchPokeDescription,
    [MOVE_OVERDRIVE - 1] = sOverdriveDescription,
    [MOVE_APPLE_ACID - 1] = sAppleAcidDescription,
    [MOVE_GRAV_APPLE - 1] = sGravAppleDescription,
    [MOVE_SPIRIT_BREAK - 1] = sSpiritBreakDescription,
    [MOVE_STRANGE_STEAM - 1] = sStrangeSteamDescription,
    [MOVE_LIFE_DEW - 1] = sLifeDewDescription,
    [MOVE_OBSTRUCT - 1] = sObstructDescription,
    [MOVE_FALSE_SURRENDER - 1] = sFalseSurrenderDescription,
    [MOVE_METEOR_ASSAULT - 1] = sMeteorAssaultDescription,
    [MOVE_ETERNABEAM - 1] = sEternabeamDescription,
    [MOVE_STEEL_BEAM - 1] = sSteelBeamDescription,
    [MOVE_EXPANDING_FORCE - 1] = sExpandingForceDescription,
    [MOVE_STEEL_ROLLER - 1] = sSteelRollerDescription,
    [MOVE_SCALE_SHOT - 1] = sScaleShotDescription,
    [MOVE_METEOR_BEAM - 1] = sMeteorBeamDescription,
    [MOVE_SHELL_SIDE_ARM - 1] = sShellSideArmDescription,
    [MOVE_MISTY_EXPLOSION - 1] = sMistyExplosionDescription,
    [MOVE_GRASSY_GLIDE - 1] = sGrassyGlideDescription,
    [MOVE_RISING_VOLTAGE - 1] = sRisingVoltageDescription,
    [MOVE_TERRAIN_PULSE - 1] = sTerrainPulseDescription,
    [MOVE_SKITTER_SMACK - 1] = sSkitterSmackDescription,
    [MOVE_BURNING_JEALOUSY - 1] = sBurningJealousyDescription,
    [MOVE_LASH_OUT - 1] = sLashOutDescription,
    [MOVE_POLTERGEIST - 1] = sPoltergeistDescription,
    [MOVE_CORROSIVE_GAS - 1] = sCorrosiveGasDescription,
    [MOVE_COACHING - 1] = sCoachingDescription,
    [MOVE_FLIP_TURN - 1] = sFlipTurnDescription,
    [MOVE_TRIPLE_AXEL - 1] = sTripleAxelDescription,
    [MOVE_DUAL_WINGBEAT - 1] = sDualWingbeatDescription,
    [MOVE_SCORCHING_SANDS - 1] = sScorchingSandsDescription,
    [MOVE_JUNGLE_HEALING - 1] = sJungleHealingDescription,
    [MOVE_WICKED_BLOW - 1] = sWickedBlowDescription,
    [MOVE_SURGING_STRIKES - 1] = sSurgingStrikesDescription,
    [MOVE_THUNDER_CAGE - 1] = sThunderCageDescription,
    [MOVE_DRAGON_ENERGY - 1] = sDragonEnergyDescription,
    [MOVE_FREEZING_GLARE - 1] = sFreezingGlareDescription,
    [MOVE_FIERY_WRATH - 1] = sFieryWrathDescription,
    [MOVE_THUNDEROUS_KICK - 1] = sThunderousKickDescription,
    [MOVE_GLACIAL_LANCE - 1] = sGlacialLanceDescription,
    [MOVE_ASTRAL_BARRAGE - 1] = sAstralBarrageDescription,
    [MOVE_EERIE_SPELL - 1] = sEerieSpellDescription,
    [MOVE_DIRE_CLAW - 1] = sDireClawDescription,
    [MOVE_PSYSHIELD_BASH - 1] = sPsyshieldBashDescription,
    [MOVE_POWER_SHIFT - 1] = sPowerShiftDescription,
    [MOVE_STONE_AXE - 1] = sStoneAxeDescription,
    [MOVE_SPRINGTIDE_STORM - 1] = sSpringtideStormDescription,
    [MOVE_MYSTICAL_POWER - 1] = sMysticalPowerDescription,
    [MOVE_RAGING_FURY - 1] = sRagingFuryDescription,
    [MOVE_WAVE_CRASH - 1] = sWaveCrashDescription,
    [MOVE_CHLOROBLAST - 1] = sChloroblastDescription,
    [MOVE_MOUNTAIN_GALE - 1] = sMountainGaleDescription,
    [MOVE_VICTORY_DANCE - 1] = sVictoryDanceDescription,
    [MOVE_HEADLONG_RUSH - 1] = sHeadlongRushDescription,
    [MOVE_BARB_BARRAGE - 1] = sBarbBarrageDescription,
    [MOVE_ESPER_WING - 1] = sEsperWingDescription,
    [MOVE_BITTER_MALICE - 1] = sBitterMaliceDescription,
    [MOVE_SHELTER - 1] = sShelterDescription,
    [MOVE_TRIPLE_ARROWS - 1] = sTripleArrowsDescription,
    [MOVE_INFERNAL_PARADE - 1] = sInfernalParadeDescription,
    [MOVE_CEASELESS_EDGE - 1] = sCeaselessEdgeDescription,
    [MOVE_BLEAKWIND_STORM - 1] = sBleakwindStormDescription,
    [MOVE_WILDBOLT_STORM - 1] = sWildboltStormDescription,
    [MOVE_SANDSEAR_STORM - 1] = sSandsearStormDescription,
    [MOVE_LUNAR_BLESSING - 1] = sLunarBlessingDescription,
    [MOVE_TAKE_HEART - 1] = sTakeHeartDescription,
    [MOVE_TERA_BLAST - 1] = sTeraBlastDescription,
    [MOVE_SILK_TRAP - 1] = sSilkTrapDescription,
    [MOVE_AXE_KICK - 1] = sAxeKickDescription,
    [MOVE_LAST_RESPECTS - 1] = sLastRespectsDescription,
    [MOVE_LUMINA_CRASH - 1] = sLuminaCrashDescription,
    [MOVE_ORDER_UP - 1] = sOrderUpDescription,
    [MOVE_JET_PUNCH - 1] = sJetPunchDescription,
    [MOVE_SPICY_EXTRACT - 1] = sSpicyExtractDescription,
    [MOVE_SPIN_OUT - 1] = sSpinOutDescription,
    [MOVE_POPULATION_BOMB - 1] = sPopulationBombDescription,
    [MOVE_ICE_SPINNER - 1] = sIceSpinnerDescription,
    [MOVE_GLAIVE_RUSH - 1] = sGlaiveRushDescription,
    [MOVE_REVIVAL_BLESSING - 1] = sRevivalBlessingDescription,
    [MOVE_SALT_CURE - 1] = sSaltCureDescription,
    [MOVE_TRIPLE_DIVE - 1] = sTripleDiveDescription,
    [MOVE_MORTAL_SPIN - 1] = sMortalSpinDescription,
    [MOVE_DOODLE - 1] = sDoodleDescription,
    [MOVE_FILLET_AWAY - 1] = sFilletAwayDescription,
    [MOVE_KOWTOW_CLEAVE - 1] = sKowtowCleaveDescription,
    [MOVE_FLOWER_TRICK - 1] = sFlowerTrickDescription,
    [MOVE_TORCH_SONG - 1] = sTorchSongDescription,
    [MOVE_AQUA_STEP - 1] = sAquaStepDescription,
    [MOVE_RAGING_BULL - 1] = sRagingBullDescription,
    [MOVE_MAKE_IT_RAIN - 1] = sMakeItRainDescription,
    [MOVE_RUINATION - 1] = sRuinationDescription,
    [MOVE_COLLISION_COURSE - 1] = sCollisionCourseDescription,
    [MOVE_ELECTRO_DRIFT - 1] = sElectroDriftDescription,
    [MOVE_SHED_TAIL - 1] = sShedTailDescription,
    [MOVE_CHILLY_RECEPTION - 1] = sChillyReceptionDescription,
    [MOVE_TIDY_UP - 1] = sTidyUpDescription,
    [MOVE_SNOWSCAPE - 1] = sSnowscapeDescription,
    [MOVE_POUNCE - 1] = sPounceDescription,
    [MOVE_TRAILBLAZE - 1] = sTrailblazeDescription,
    [MOVE_CHILLING_WATER - 1] = sChillingWaterDescription,
    [MOVE_HYPER_DRILL - 1] = sHyperDrillDescription,
    [MOVE_TWIN_BEAM - 1] = sTwinBeamDescription,
    [MOVE_RAGE_FIST - 1] = sRageFistDescription,
    [MOVE_ARMOR_CANNON - 1] = sArmorCannonDescription,
    [MOVE_BITTER_BLADE - 1] = sBitterBladeDescription,
    [MOVE_DOUBLE_SHOCK - 1] = sDoubleShockDescription,
    [MOVE_GIGATON_HAMMER - 1] = sGigatonHammerDescription,
    [MOVE_COMEUPPANCE - 1] = sComeuppanceDescription,
    [MOVE_AQUA_CUTTER - 1] = sAquaCutterDescription,
    [MOVE_BLAZING_TORQUE - 1] = sBlazingTorqueDescription,
    [MOVE_WICKED_TORQUE - 1] = sWickedTorqueDescription,
    [MOVE_NOXIOUS_TORQUE - 1] = sNoxiousTorqueDescription,
    [MOVE_COMBAT_TORQUE - 1] = sCombatTorqueDescription,
    [MOVE_MAGICAL_TORQUE - 1] = sMagicalTorqueDescription,
    [MOVE_PSYBLADE - 1] = sPsybladeDescription,
    [MOVE_HYDRO_STEAM - 1] = sHydroSteamDescription,
    [MOVE_BLOOD_MOON - 1] = sBloodMoonDescription,
    [MOVE_MATCHA_GOTCHA - 1] = sMatchaGotchaDescription,
    [MOVE_SYRUP_BOMB - 1] = sSyrupBombDescription,
    [MOVE_IVY_CUDGEL - 1] = sIvyCudgelDescription,
    [MOVE_ELECTRO_SHOT - 1] = sElectroShotDescription,
    [MOVE_TERA_STARSTORM - 1] = sTeraStarstormDescription,
    [MOVE_FICKLE_BEAM - 1] = sFickleBeamDescription,
    [MOVE_BURNING_BULWARK - 1] = sBurningBulwarkDescription,
    [MOVE_THUNDERCLAP - 1] = sSuckerPunchDescription,
    [MOVE_MIGHTY_CLEAVE - 1] = sFeintDescription,
    [MOVE_TACHYON_CUTTER - 1] = sTachyonCutterDescription,
    [MOVE_HARD_PRESS - 1] = sWringOutDescription,
    [MOVE_DRAGON_CHEER - 1] = sDragonCheerDescription,
    [MOVE_ALLURING_VOICE - 1] = sAlluringVoiceDescription,
    [MOVE_TEMPER_FLARE - 1] = sTemperFlareDescription,
    [MOVE_SUPERCELL_SLAM - 1] = sSupercellSlamDescription,
    [MOVE_PSYCHIC_NOISE - 1] = sPsychicNoiseDescription,
    [MOVE_UPPER_HAND - 1] = sUpperHandDescription,
    [MOVE_MALIGNANT_CHAIN - 1] = sMalignantChainDescription,

    [MOVE_SWIRLING_BLADE - 1] = sNullDescription,
};
